...
| **3** | [`sprint-6.md`](../../../sprints/done/sprint-6.md), the `## Status` row whose Brief cell links `0188-repair-the-five-live-ownership-fact-defects` | *"**D1** `claude/scaffold/CLAUDE.md`'s producer row omits `/fkit-task-brief` — a **declared mirror that ships into every consuming project's root `CLAUDE.md`**"* | `:241` | ✅ **yes — verified present, and now false** |
