...
## Step 10 — process the stateful review, round 2: R9–R13 (2026-09-04)

⭐ **Coverage this round: `reasoning-only second opinion`** — ADR-042's **normal** state, ⛔ **not** a
degradation. Round 1's `Codex unavailable` is **superseded, not amended**; coverage is per round.
⚠️ **Codex measured nothing** — every one of its commands was a read — so all execution evidence in this
ledger remains the Claude reviewer's or mine.

⭐ **Waiting for the second opinion was worth it, on the evidence.** It produced five findings round 1
missed, **two of them substantive false claims inside notes this sweep itself shipped**, and **one that
corrects my own round-1 disposition**. ⛔ **Nothing in the suite could have caught any of them.**

### ⛔ Three of the five are defects I authored — and two are the exact failure I was warned about

`0281`'s brief says, in terms: *"A correction that overstates is a worse defect than the stale claim it
replaces."* ⛔ **I then wrote overstatements into two ADR correction notes.** Recorded plainly:

- **R9** — my ADR-003 note asserted `bin/release.mjs` *"runs `npm test` **before every release**"* and
  that *"**there is no warn-and-continue path**"*. **Both false.** `--no-test` skips the gate (its help
  says *"SHIPS AN UNVERIFIED TREE"*) and the `else` branch warns and **proceeds**. ⭐ **Root cause, worth
  more than the fix:** I generalised from the source's own loose header comment (*"every run executes
  `npm test` first"*) instead of reading the `doTest` gate two dozen lines below — **a loose comment
  propagating into a decision record, which is precisely the defect class this sprint exists to end.**
- **R10** — my ADR-037 note defined the proxy as *"the spawn prompt **contains the bytes**"* and called
  it *"mechanically checked today"*, while enumerating *"Four limits… Present tense without these
  overstates."* ⛔ **A fifth limit was missing:** the hook skips byte-containment for a **declared
  pointer-only spawn** and compares the hash alone. **My round-1 pass read the hook's header comments
  and never reached the `declared` branch.**
- **R11** — A4 lacked its own byte-identity statement. ⛔ **This corrects my own round-1 R6, where I
  accepted the reviewer's claim that *"A1–A4 and B4 are complete"* without checking it** — the exact
  inheriting Step 3 forbids.

### Decision log — round 2, per the ADR-019 audit obligation

| Finding | What changed | Why it qualified |
|---|---|---|
| **R9** | ADR-003 note item 1 rewritten: gate runs **by default** and aborts on red; `--no-test` skips it, warns loudly, proceeds, and is never a default | Verified `CORRECT` against `bin/release.mjs` (both halves) and against `351bea3` (the line is the sweep's own); localized to lines I authored; in-plan |
| **R10** | ADR-037 note now lists **Five limits**, the new one naming the declared-pointer-only skip; the proxy definition qualified where it is defined | Verified `CORRECT` against `carry-check-hook.mjs`'s `declared` gate and its own comment; localized to lines I authored; in-plan |
| **R11** | A4 gained its own dated byte-identity statement | Verified `CORRECT`; mechanical, append-only, inside `0335`'s own stated requirement |
| **R12** | Subject-B preamble corrected: **three** notes below, the fourth in `0327`'s `worklog.md`, named | Verified `CORRECT`; mechanical, corrects a false inventory |
| **R13** | Added an attribution paragraph separating the two bases — the fixture proves the **guard is skipped**; session start and *"agent not found"* rest on the code-confirmed finding | Verified **PARTIALLY CORRECT**; I derived `low` myself and agree with the downgrade from Codex's `medium` |

**Obvious-winner calls made unattended: `none`.** All five were verified-`CORRECT` (R13 partially),
mechanical or localized, and inside the approved plan.

### Invariants re-proved after the round-2 edits

```
ADR-003   pre-sweep (351bea3) lines now missing: 0
ADR-037   pre-sweep (351bea3) lines now missing: 0
0327 ledger  pre-sweep lines missing: 0 · all THREE party sections BYTE-IDENTICAL
my own ledger: reviewer lines removed: 0 · every committed reviewer line still present
```

⭐ **The byte-level section-ownership proof I could NOT give in round 1, I can now give.** `review.md`
was untracked then; it is committed at `cf289c2` now, so the *Reviewer findings* section has a real
baseline. **Zero reviewer lines removed**; the section's only delta is the reviewer's own R9–R13 append.

### ⚠️ The line-break trap bit for the THIRD time in this task

My plain grep for the fourth subject-B note returned **zero** — the phrase wraps a line break. The same
trap produced my `0312` false negative and the reviewer's `0299` one. ⛔ **A plain grep is not an
absence check in this repo**; the whitespace-normalised form is, and it is what found the note.

### Ledger `Status:` — set to `closed-out`, and why that is now truthful

⭐ **Owner ruling N4's condition is discharged:** the second opinion exists and coverage is
`reasoning-only second opinion`, the normal state. Every novel finding across **both** rounds is
dispositioned — twelve `✅ done`, one `won't fix (frontier)` — two accepted residuals carry their
**What / Why / Re-raise only if**, and nothing blocking remains on the work product (ADR-034).
⛔ **In round 1 `closed-out` would have been false and I refused it then**; the fact that changed is the
second opinion, not my confidence.

⚠️ **The honest caveat that goes with it:** round 2 found two substantive false claims round 1 missed
entirely, both in shipped artifacts, neither reachable by any test. ⛔ **That is evidence about how
little the suite covers this class of work — not evidence that a third round would converge.** The
reviewer's convergence call was *act, then close*. I have acted.

### Step 10 verification — measured on the post-round-2 tree (2026-09-04)

```
guards:            41 tests · 41 pass · 0 fail · 0 skipped
                   citation: total 166 = exempt 166 + residual 0
                   link:     857 files · 3267 targets · 0 broken · 6 named-exempt (set still exactly six)
unit suite:        833 tests · 833 pass · 0 fail · 0 cancelled · 0 skipped · 0 todo · 24 suites
prove-red.sh:      ✓ hard gate PASSED — 28 mutations red at their named assertion · 13 clean-copy steps green
failure markers:   0                       exit code: 0
HEAD cf289c2 · staged 0 · renames 0 · wiki-vault modified 0 · .claude/ untouched · init not run
```

⛔ **And it proves nothing about R9–R13.** All five were false claims or attribution defects **inside
prose** — two of them in shipped ADR correction notes. **No test reads any of them.** What proves them
is the per-finding re-verification recorded above, each run as a command against the actual source:
`bin/release.mjs`'s `doTest` gate and `else` branch for R9, `carry-check-hook.mjs`'s `declared` gate for
R10, and the ledger text itself for R11–R13.

⚠️ **Round-2 change surface (the sweep is committed at `cf289c2`, so these are round-2 edits only):**

```
adr-003  +7  / −2      adr-037  +10 / −2      0327 review.md  +16 / −2
0357 review.md  +42 / −8      0357 worklog.md  +79 / −0
```

⚠️ **The link guard now scans 857 files, up from 855** — that is the owner's own new ADR-046 and task
`0361` plan, neither of them mine. **Expected, not drift.**
