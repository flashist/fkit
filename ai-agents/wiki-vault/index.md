...
- [[tasks/sprint-2-remove-omnigent]] — Remove Omnigent, land Claude-native as the only runtime (**138 done · 1 backlog · 5 cancelled · 45 moved — of 189**, re-derived against `dashboard.sh` 2026-08-06; grew 22 → 189, and long since past its original theme). ⚠️ **`1 backlog` is NOT "one task left"** — it is one row still *scoped* to Sprint 2 after `0210`'s 45-row de-scope; the open work lives on the **Backlog board**, which a default `/fkit-status` call does not report. ⚠️ **It is growing faster than it closes** — the 2026-08-04/05 run shipped **6** and filed **11**. **25 consecutive agent-closed rows**, none counted by anything. ⚠️ **81% closed is now a structural problem**: 16 of 29 open rows were **unreachable by rank**, and the rollover decision (`0185`) is **deferred by the owner**
...
- [[tasks/decide-whether-a-spawn-instruction-may-override-a-skill-rule]] — Task `0158` → **ADR-037**. Ruling only; **no skill, agent definition or source file was edited**, and both clauses are still unwritten. Corrected its own brief in **four** places, including the stale `:245-249` pointer and a verbatim quote that **silently dropped *"It is open to an owner override"***. ⚠️ Its counterfactual sentence **shipped wrong in the first draft** — followed literally it re-points the frozen ledger the ADR exists to protect — and its budget arithmetic was corrected in review. ⚠️ **Its ledger carries two record defects** (`Status: in-review` never flipped; residuals with no per-entry re-raise trigger), owner-gated to `0201`
...
- [[tasks/decide-whether-process-review-is-always-the-coder-or-the-architect-gains-the-skill]] — Task `0200` → **the Process-review step's role is fixed by the skill the step runs, not by who wrote the deliverable — always `@fkit-coder`.** Rejects widening `skills_for_role()`: the grant is **total or absent**, and the premise reaches **four of seven roles**. ⚠️ **The brief's framing was wrong** — prose and hook never disagreed, **the driver departed from both** — which changes the question. ⚠️ **Round 1's *"method" → "run the skill"* answer was REVERSED**: the invocation form would **re-impose the per-round owner gate ADR-019 rejected**; the real defect is that the row **names the method without enumerating it**, and `0195` measured the cost — **four procedure steps never ran**. Detector must be a **PAIR**; **the rule stays PROSE, not prevention**. ADR-038 is `0222`, unwritten
