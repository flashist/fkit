...
- **The rule is checkable**, so a guard becomes possible — but **none is built here**. The proposed `test/closed-rank-immutability.test.js` is a **diff** check across a commit range (no property of a single board file reveals that a closed row was renumbered), ranked LOW, and **would be red on the commit that filed `0174`** — which is why it needs a baseline decision first.
  - *✅ Dated correction 2026-08-06 (recorded by the 2026-08-07 sync; original above byte-identical):* **the guard is now BUILT** — [[tasks/build-the-closed-rank-immutability-guard]] (`0182`, Sprint 3 P2). The baseline question was **dissolved rather than answered**, on owner-delegated architect rulings: baseline = `HEAD`, scope = the transition in progress (plus a `HEAD`↔`HEAD^` leg), **no history range** — so the `0174` filing commit is not red and not exempted; it is **outside the guard's stated scope**. The "would be red" replay survives as a fixture test over the two `0174` revisions. ⛔ The guard is **not continuous protection**: no CI runs it, and a breach committed with no test run in the window is never caught.
    - *✅ Dated correction 2026-08-13 (the `0282` resync; the line above is left byte-identical):* **"no CI runs it" is now FALSE** — `.github/workflows/test.yml` landed 2026-08-12 ([[tasks/gate-releases-so-an-untested-tree-cannot-ship]], `0256`) and runs `npm test`, which includes this guard, on every push to `main` and every PR. ⚠️ **The `⛔ not continuous protection` verdict SURVIVES on a narrower reading:** leg 2 sees `HEAD`↔`HEAD^` only, so a CI run over a multi-commit push inspects one transition and a middle-commit breach is still missed. **Also note the 2026-08-06 owner ruling quoted on the `0182` page — *"No CI planned."* — was reversed on 2026-08-08.**
...
- [[tasks/sprint-5-fix-what-a-real-project-found]] — ⚠️ *Added 2026-08-13:* **three separate rank events on one board, each checked against this ADR and each resolving differently.** (1) A spawned producer **declined** an owner-relayed mid-board insertion and appended instead — ⛔ *not precedent*. (2) An owner-ruled **MOVE** of existing rows was **permitted**, because nothing entered the numbering, no closed row was renumbered, and it reached exactly the top of its contiguous open run — *had the ruling named a rank one place higher it would have been refused*. (3) A new row appended at the bottom **against its own stated merit**, because nine closed rows sat below every reachable position. ***The clearest worked example of this ADR in force.***
- [[tasks/make-the-lockdown-guard-case-test-filesystem-independent]] — `0283`: **a new row that appended against its own stated merit** — on merit it belonged at the **top** of the board (the only red on `main`), unreachable behind nine closed rows. ***The accepted cost of this ADR, paid in public and flagged for owner confirmation.***
- [[tasks/gloss-the-moved-to-sprint-n-row]] · [[tasks/correct-the-five-remaining-prose-sites-that-state-the-glob-as-the-mechanism]] — two rows moved by the 2026-08-11 owner-ruled re-rank; the second exposed a **near-miss** — it now outranks a task it cites, safe only because a *different* owner ruling had removed the sequencing dependency
- [[tasks/sprint-6-repair-the-record-the-board-rests-on]] — ⚠️ *Added 2026-08-22:* the board that opened **unranked** because a spawned producer may not rank, was ranked `P1`–`P18` by owner ruling the same day, and then took `P19` as an **append, never an insertion**
- [[tasks/record-the-canonical-merit-statement-form-in-the-convention-page]] — ⚠️ *Added 2026-08-22:* task `0178`, which wrote this ADR's merit-statement form into `conventions/priority-is-rank-not-identity.md`, dual-homed
- [[tasks/refuse-the-destructive-claude-refresh-through-a-symlink-and-correct-the-only-destructive-claim]] · [[tasks/repair-the-moved-folders-own-self-locators-in-task-done]] — *added 2026-08-29:* `0327` and `0325`, the two rows inserted mid-board on Sprint 6 — ⭐ **each landing in exactly the shape this ADR's own residual-risks section names by hand**: a closed run contiguous at the top, so the insertion below it renumbered nothing closed
- [[tasks/write-the-durable-citation-anchors-convention-page]] — *added 2026-08-29 (lint):* `0171`, one of the Sprint 6 rows this ADR's rank rules govern
- [[decisions/adr-046-a-sprint-board-may-be-committed-unranked-and-an-erased-rank-flags]] — ⭐ *Added 2026-09-05 (`0358`, sweep C):* the adjacent rank-integrity ruling — an unranked cell becomes committable, and erasing a **closed** row's frozen rank becomes an explicit violation
- *Added 2026-09-10 (sync `cf289c2`→`b4a1a52`):* [[tasks/cut-the-v0-3-0-release-and-hand-archive-sprint-7]] — task `0360`, the v0.3.0 cut and Sprint 7's hand-archive · [[tasks/settle-whether-a-sprint-board-may-be-committed-unranked]] — task `0361`, which produced ADR-046 · [[tasks/sprint-7-stop-manufacturing-record-repair-rows]] — Sprint 7, the board that attacked the record-repair rate (archived 2026-09-08)
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/sprint-8-give-sprints-the-lifecycle-tasks-already-have]] — ⭐ **the owner-ruled two-row SWAP of 2026-09-10 (`S6`)**: the five untouched rows keep both their ranks and their relative order, which is what keeps it inside the exception
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/backfill-a-sprint-status-onto-every-existing-sprint-plan]] — task `0340`, the row that moved `P6`→`P3`
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/sprint-9-settle-architecture-mds-truth-and-sweep-the-citation-rot]] — a board that records its rank/merit disagreement rather than renumbering
