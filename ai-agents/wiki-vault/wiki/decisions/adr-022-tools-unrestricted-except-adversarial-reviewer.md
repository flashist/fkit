...
3. **Exactly one wall protects a genuine, checkable invariant: the adversarial reviewer's** — no Write/Edit/Agent is what makes *"the independent second opinion never touched the code it judges"* a structural fact. Every other wall is accidental or product-discipline (most notably the lead's router-not-doer guardrail — ⚠️ *that guardrail was itself reversed on 2026-07-22, see the note under Decision 4*) — philosophy, not safety.
...
4. **Prompt-level role contracts are unaffected** — relaxing tools does not relax contracts (the lead is still "not a doer", the reviewer "review-only", etc.).
   > ⚠️ **The lead half of this example is no longer true — annotated 2026-07-29, not rewritten.** *"the lead is still 'not a doer'"* was correct when this ADR was written (**2026-07-18**) and was **reversed on 2026-07-22** by [[decisions/adr-031-fkit-lead-becomes-the-orchestrating-front-door]]: `fkit-lead` is now an **orchestrating conductor** that spawns and drives typed peers and owns `/fkit-sprint-ship-loop`. **This ADR's own decision still stands** — relaxing tools did not relax the contract; the contract was changed later, by a different ADR. The reviewer's *"review-only"* example is **unaffected and still current**. Verified against the tree 2026-07-29: `claude/agents/fkit-lead.md` carries no *"not a doer"* / *"does no work"* assertion, and `claude/skills/fkit-sprint-ship-loop/SKILL.md:29` describes lead as *"a **driver**, not a doer"* — a different claim (it delegates rather than performs), **not** the retired router-only stance.
...
- [[decisions/adr-042-a-codex-review-is-reasoning-only-and-reports-must-say-so]] — ⛔ ***Added 2026-08-13: this is the decision that gives up this ADR's one surviving structural wall.*** ADR-042's **D2** — owner-ruled 2026-08-11 **against the architect's recommendation, knowingly and provisionally** — moves the Codex sandbox to `workspace-write` at the repo root. Once it ships, *"the independent second opinion never touched the code it is judging"* stops being a **structural fact** and becomes **a promise again**, held only by prompt instruction — and a violation would be **invisible**, because no hook sees it, no test watches the tree, and the review's own output would not mention it. ⚠️ **Verified 2026-08-13: D2 is NOT built** — all five sites under `claude/` still read `read-only` and task `0273` is open — **so this ADR's wall is intact today.** The owner reserved two exits (harden, or disable write access entirely)
- [[tasks/repair-the-five-live-ownership-fact-defects]] — *added 2026-08-29:* `0188` finding **R3**, which found docs still claiming a per-role tool allowlist this ADR retired for every role but one
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/the-two-citation-sweeps-architecture-md-and-the-repo-wide-adr-line-class]] — task `0393`: ⭐ **this page's own `ADR-008:85` citation was re-resolved and LEFT ALONE** — it still lands, and *don't-repair-what-works* applied
