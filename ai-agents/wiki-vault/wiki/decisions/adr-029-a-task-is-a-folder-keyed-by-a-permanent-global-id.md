...
- [[tasks/remediate-the-dead-brief-paths-in-closed-review-ledger-headers]] — *added 2026-08-29:* `0168`; ⭐ **two of its three dead-path variants predate this migration**, so repairing them means translating a pre-`0062` flat name to its `NNNN` folder, not re-pointing a path
- [[decisions/adr-046-a-sprint-board-may-be-committed-unranked-and-an-erased-rank-flags]] — ⭐ *Added 2026-09-05 (`0358`, sweep C):* rests on this ADR's *"the Priority cell is mutable board rank and never identity"* to justify accepting `—` with **no ordering semantics**
- *Added 2026-09-10 (sync `cf289c2`→`b4a1a52`):* [[tasks/build-the-coordination-citation-policy-guard]] — task `0176`, the coordination-citation policy guard · [[tasks/the-throughput-counter-created-vs-closed-per-iso-week]] — task `0359`, the throughput counter
