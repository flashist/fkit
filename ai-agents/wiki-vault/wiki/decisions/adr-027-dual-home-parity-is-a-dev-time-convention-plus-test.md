...
2. **Then build the mechanical parity test** — `test/dual-home-parity.test.js` under ADR-014 (`node --test`, zero devDeps), asserting every fkit-authored dual-homed file is byte-identical, with an explicit exception list. **Preceded by a reconciliation change** that byte-aligns the six drifted files — without it the test fails on day one.
   > ⚠️ **Dated correction 2026-08-02 — the byte-align mandate in this Decision was OVERRULED BY THE OWNER, and the sentence above is left byte-identical as the record of what the ADR decided on 2026-07-19.** Task `0132`'s sweep disproved this ADR's premise: the drifted scaffold `conventions/*` files are **not stale copies left behind** — **five of the six are deliberate, de-fkit-ified, audience-adapted rewrites** for a consuming project's reader. **Owner ruling 2026-08-01, Option B:** *"audience-adapted"* is a legitimate **third kind** alongside ✅ must-match and ⛔ never-sync, and **byte-aligning live → scaffold is rejected as a product regression** — it would ship fkit's own incident narrative and **4 verified-broken relative links** into every new project. **So §Decision 2 as written is a live record instructing a future implementer to ship a regression.** ⚠️ **The ADR's real defect is its EVIDENCE, not its arithmetic:** *"six drifted files"* is a `diff -rq` count, and **a `diff` count cannot distinguish a stale copy from a deliberate adaptation**. The figure is stale **in kind, not in count** — all six still differ, and **none** were fixed by `0043`/`0077`/`0086`. **This ADR is NOT yet amended on disk** — amending it is an architect act, filed as task `0186` and **still open**. See [[tasks/reconcile-dual-homed-file-drift-live-vs-scaffold]].
   > ✅ **Both halves of this Decision have now SHIPPED** — reconciliation ([[tasks/reconcile-dual-homed-file-drift-live-vs-scaffold]], task `0132`, 2026-08-01) then the test ([[tasks/build-dual-home-parity-test]], task `0133`, 2026-08-02), in the order §Decision 3 makes binding. **The authoritative exception list is `test/dual-home-parity-exceptions.mjs` — 26 entries, each with its own specific reason.** Also newly recorded: **`decisions/` and `reports/` are NOT part of the dual-homed surface, so no ADR is ever a drift event**, and `claude/scaffold/universal-rules.md` is **single-homed** and outside it.
...
- [[tasks/reconcile-dual-homed-file-drift-live-vs-scaffold]] — task `0132`, the reconciliation this ADR ordered — **and the sweep that disproved its premise**
- [[tasks/build-dual-home-parity-test]] — task `0133`, §Decision 2's test, **built 2026-08-02**
- [[decisions/adr-036-the-skill-ownership-site-inventory-is-a-declared-registry]] — copies this ADR's shape almost exactly: convention + hand-rolled test + machine-readable exception list **with mandatory reasons**
...
- [[tasks/gloss-the-moved-to-sprint-n-row]] — task `0268` (2026-08-13): ⚠️ **a live instance of this regime's sharpest hazard.** The file it edits is a **declared `audience-adapted` exception**, so the parity test **subtracts it from the check** — both copies exist, the target line is byte-identical in both, **and nothing will tell you if you edit one and not the other.** *The exception list excludes precisely the files most likely to be edited on one side only*
- [[tasks/update-the-docs-for-the-structure-check-capability]] — `0248`: dual-homed scaffold pointers landed in **both** copies, with the parity test as the check
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/add-dual-home-paritys-missing-accepted-drift-row-and-correct-its-count]] — task `0389`: ⛔ **the mirror's completeness claim was wrong by two and one entry had no row at all**; ⚠️ **no test pins the count**
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/teach-the-roles-what-current-sprint-means]] — task `0339`, which ⭐ **fully honoured the rule** and still left the page's number staler
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/architecture-md-prose-repair-9-1-inventory-occurrence-b-and-9-5]] — task `0392`: ⭐ **§9.5's accurate dual-home paragraph SURVIVED the section's rewrite** — collision `X4` had flagged that it must not die as collateral if §9.5 went
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/sweep-the-repo-only-claude-path-form-out-of-installed-facing-prose]] — task `0390`: **Class B edits touched dual-homed scaffold files**, so the parity suite is one of its verification steps
