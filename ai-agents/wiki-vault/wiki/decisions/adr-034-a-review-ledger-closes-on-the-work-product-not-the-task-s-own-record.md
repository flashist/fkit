...
- [[decisions/adr-042-a-codex-review-is-reasoning-only-and-reports-must-say-so]] — ⚠️ *Added 2026-08-13:* **this ADR is why two ledgers that provably overstate their coverage were left unedited.** `0259` claimed *"Coverage is FULL"* and `0264` claimed *"not partial"* while contradicting itself in the same file; both were calibrated on the old binary vocabulary. **A ledger closes on the work product, so neither was corrected**, and ADR-042 explicitly declines to file the follow-up. *A concrete case of this ADR's accepted cost — a closed ledger is clean to a work-product standard, not a record-perfect one*
- [[tasks/widen-task-done-to-repair-a-brief-that-contradicts-a-landed-close]] — *added 2026-08-29:* `0229`, whose worklog-attribution slip was noted under this ADR rather than raised as a finding
- [[decisions/adr-045-an-in-flight-review-finding-terminates-in-the-ledger-not-a-new-task]] — ⭐ *Added 2026-09-05 (`0358`, sweep C):* the in-flight lane, which **relaxes nothing** in this ADR's work-product bar. ⚠️ Its condition B is a **per-file** test while this ADR's boundary is **per-site** — the ADR names that mismatch rather than leaving it to discovery
- *Added 2026-09-10 (sync `cf289c2`→`b4a1a52`):* [[tasks/adr-the-narrow-in-flight-review-fix-lane]] — task `0352`, which produced ADR-045 · [[tasks/sweep-a-the-citation-rot-class-one-verified-pass]] — task `0356`, Sweep A — the citation-rot class in one pass · [[tasks/sweep-b-the-single-site-correction-notes]] — task `0357`, Sweep B — the single-site correction notes
