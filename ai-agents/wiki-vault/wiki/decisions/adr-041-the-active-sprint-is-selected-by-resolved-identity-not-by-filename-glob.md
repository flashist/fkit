...
- *Added 2026-09-16 (sync `b4a1a52`→`81f1429`):* [[decisions/adr-047-a-sprint-has-an-explicit-status-and-current-means-every-in-progress-sprint]] — ⛔ **SUPERSEDES THIS ADR IN PART** (scoped in its §8). ⭐ **What survives untouched and in force:** the candidate set, the identity ladder, the same-identity tie-break, the `Backlog` token, and the one-grammar-one-implementation constraint. What changes is the *selection rule* — every `In progress` sprint is current, and where one board must be chosen the default flips to the **lowest-ordered**, not the highest-numbered
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/record-the-sprint-lifecycle-adr-047]] — task `0337`, which **superseded §1.4 in part**; ⭐ this ADR's `:330-332` names the owner report that triggered it
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/give-the-selector-a-status-rung-and-a-lowest-first-choice]] — task `0338`, which flipped the ordering from highest to lowest and added the status rung
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/pin-the-five-unpinned-behaviors-in-the-sprint-identity-grammar]] — task `0271`, pinning `select-active` **before** `0338` rewrote it
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/sprint-8-give-sprints-the-lifecycle-tasks-already-have]] — the board that shipped §1.4's supersession
