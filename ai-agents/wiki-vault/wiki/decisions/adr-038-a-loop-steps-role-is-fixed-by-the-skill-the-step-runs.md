...
- [[decisions/adr-040-a-plan-s-sprint-identity-is-a-whole-h1-segment-never-a-substring]] · [[decisions/adr-041-the-active-sprint-is-selected-by-resolved-identity-not-by-filename-glob]] — ⚠️ *Added 2026-08-13:* both reuse this ADR's **prose-not-prevention residual shape** by name — the identity grammar is *documented and detected, never prevented*. ADR-041 also cites this ADR for **why the sprint loop cannot run a `fkit-wiki`-owned row**: the loop never reads `## Owner`, so its Build step spawns `@fkit-coder`, which is barred from the vault — the reason three Sprint 5 rows are excluded from the loop run
- [[tasks/sprint-5-fix-what-a-real-project-found]] — ⚠️ **this ADR is the reason three Sprint 5 rows are excluded from the `/fkit-sprint-ship-loop` run.** Two are `## Owner: fkit-wiki`, but **the loop never reads `## Owner`** — its Build step spawns `@fkit-coder`, barred from the vault — so the loop would stall on a refusal or breach ADR-005. ⚠️ **An exclusion from one loop run, NOT a block, deprioritisation or descope**
- [[tasks/the-2026-08-13-vault-resync-chain]] — why two of its six rows were held out of the `/fkit-sprint-ship-loop` run, and shipped through spawned `@fkit-wiki` librarians instead
- [[tasks/wiki-ingest-of-adr-043-claude-is-not-a-structure-conformance-surface]] — ⚠️ *Added 2026-08-14:* task `0293`, another `## Owner: fkit-wiki` row this ADR keeps out of the loop. It ran in a spawned librarian instead and closed 2026-08-14 — ⚠️ **an exclusion from the loop and from nothing else**
- [[decisions/adr-044-build-role-follows-the-deliverables-skill-vault-rows-skip-at-step-1]] — *added 2026-08-29:* the companion that fills the Build-role gap this ADR left to the loop table. ⛔ **This ADR is NOT amended and NOT superseded**; ADR-044 names its own Plan-step clause as an owner-ruled scoped exception, and inherits this ADR's closeout clause
- [[tasks/enumerate-the-process-review-rows-method-steps-and-give-the-row-its-reason]] — *added 2026-08-29:* `0223`, which put this ADR into the loop's Process-review row as its reason clause
- [[tasks/decide-how-the-ship-loop-handles-a-non-coder-owned-task-row]] — *added 2026-08-29:* `0270`, which ⭐ **discharged this ADR's closeout clause explicitly before weighing any option** — barred for Process-review on the hook mechanism, and a misnomer for Build
- *Added 2026-09-10 (sync `cf289c2`→`b4a1a52`):* [[tasks/note-adr-044s-oracle-rule-onto-0224-and-0225]] — task `0347`, ADR-044's oracle rule noted onto `0224`/`0225` · [[tasks/sweep-b-the-single-site-correction-notes]] — task `0357`, Sweep B — the single-site correction notes
