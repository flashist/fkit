...
- [[tasks/build-the-pretooluse-task-carry-check-hook-and-its-tests]] · [[tasks/repair-the-five-live-ownership-fact-defects]] · [[tasks/build-wiki-flag-convention-test]] · [[decisions/adr-044-build-role-follows-the-deliverables-skill-vault-rows-skip-at-step-1]] — *added 2026-08-29:* `0204` reuses this hook's deny shape for a third `PreToolUse` entry; `0188` repaired the ownership facts that describe it (⛔ including a residual that the docs overstate its reach — **it allows every non-`fkit-*` skill with no check**); `0154` records that ⛔ **this hook never opens a `SKILL.md`**, which is why the wiki flag block was unenforced; ADR-044 notes a mis-staffed loop step never reaches it at all
...
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/build-the-producer-only-sprint-movers]] — task `0341`: ⭐ **two new skills enforced with NO change to `skill-ownership-hook.sh`** — adding them to `skills_for_role()` is the enforcement, and the `MOVERS` invariant grew to four
