...
- **Both are new coder write targets** — a deliberate, bounded widening of the coder's write surface (previously: source + the *Coder response* section of the review ledger). The coder still **never** writes the wiki, and these files are **not** task briefs — the backlog/done/cancelled move rule does not apply to them. *(Dated annotation, 2026-07-29: this sentence originally read **"the owner-only … move rule"**, true when ADR-020 was written. The modifier is stale twice over — ADR-025 made the movers any-role on 2026-07-18, and [[decisions/adr-033-task-movers-are-producer-only-reversing-adr-025]] made them **`fkit-producer`-only** on 2026-07-23. **The point of the sentence is unaffected**: these files are not briefs, so no mover rule reaches them, whoever owns it. ADR-020 itself is **not** amended by ADR-033 — only this incidental modifier was wrong.)*
...
- [[tasks/decide-what-the-sprint-driver-does-when-a-spawned-worker-dies]] — task `0167`, whose resume doctrine operates over these artifacts. ⚠️ **Its instance 3 is the limit of the scheme:** every artifact that task produced was **untracked**, so `git status` carried no landing signal and **there was no history to attribute the edits to**
- [[tasks/add-adr-037s-worker-side-precedence-clause-to-the-universal-rules-block]] — task `0190`, whose residual 3 leans on this ADR: ADR-037 §2's audit **venue** was not carried into the rules block, so **the pointer was lost and the duty was not**
- [[tasks/add-adr-037s-driver-side-clause-to-the-sprint-ship-loops-hard-rules]] — task `0191`, whose decision log records `none` explicitly — *so an empty log is not mistaken for a forgotten one*
