...
- [[tasks/gate-releases-so-an-untested-tree-cannot-ship]] — task `0256` (2026-08-12). ✅ *Added 2026-08-13:* **this ADR's §Context — *"no CI, no test suite, no `.github/`"* — is a statement of fkit's STARTING condition and is correct as written; do not edit it.** What has changed is the present: the suite is **20 `test/*.test.js` files plus `prove-red.sh`**, and **`.github/workflows/test.yml` now runs it on every push to `main` and every pull request**, alongside a blocking `npm test` gate in `bin/release.mjs`. ⚠️ **This reverses a later owner ruling of 2026-08-06 (*"No CI planned."*).** ✅ **ADR-014's zero-devDependency bar is UNAMENDED and was honored** — the workflow deliberately uses no `npm ci` and no dependency cache, because there is nothing to install
- [[tasks/make-the-lockdown-guard-case-test-filesystem-independent]] — task `0283`: fkit's **first-ever CI run went red** and found a genuine test defect — an assertion that was really checking *"the filesystem folded case for me"*, invisible on macOS. ⚠️ **Evidence for this ADR's own black-box thesis: a hand-run suite on one developer's machine cannot see a platform-dependent assertion.**
- [[tasks/build-the-pretooluse-task-carry-check-hook-and-its-tests]] · [[tasks/release-mjs-with-branch-other-commits-and-tags-head-but-pushes-a-different-ref]] · [[tasks/repair-the-moved-folders-own-self-locators-in-task-done]] — *added 2026-08-29:* `0204` and `0300` added their checks with **no new dependency**; `0325` shipped **no test at all**, ⭐ **stating the honest reason rather than letting it pass unremarked** — this toolchain can assert what a procedure file *says*, never that an agent followed it
- *Added 2026-09-10 (sync `cf289c2`→`b4a1a52`):* [[tasks/build-the-link-resolution-guard]] — task `0354`, the link-resolution guard · [[tasks/settle-whether-a-sprint-board-may-be-committed-unranked]] — task `0361`, which produced ADR-046 · [[tasks/the-throughput-counter-created-vs-closed-per-iso-week]] — task `0359`, the throughput counter
