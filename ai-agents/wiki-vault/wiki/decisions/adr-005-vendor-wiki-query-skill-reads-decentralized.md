...
- [[tasks/wiki-ingest-of-adr-043-claude-is-not-a-structure-conformance-surface]] — ⚠️ *Added 2026-08-14:* task `0293` closed, the row this rule forced into existence. Its close is also a worked example of the **flag-don't-close** half of the same boundary: the librarian flagged *"ready to close"* and a **producer** moved the file (ADR-033)
- [[tasks/the-2026-08-14-retroactive-review-corrections]] — ⚠️ *Added 2026-08-14:* tasks `0291` and `0295`. Two more rows that exist **only** because this rule holds: the reviews that found the defects could not repair them, and the roles that wrote the prose could not follow their own work into the vault. ⚠️ **`0297` is a third**, and its brief says so in terms — vault writes are `fkit-wiki`'s exclusively, and the sprint loop's Build step is fixed to `@fkit-coder` (ADR-038), so the row is **excluded from the loop and from nothing else**
- [[tasks/rewrite-the-false-no-ci-paragraph-and-fabricated-citation-in-fkit-wiki-lint]] — ⚠️ *Added 2026-08-22:* task `0280`, a repair to the wiki role's own shipped lint procedure
- [[tasks/the-2026-08-15-done-in-fact-wiki-closes]] — ⚠️ *Added 2026-08-22:* tasks `0206` and `0238`, both filed **because** this ADR bars every other role from repairing the vault
- [[decisions/adr-044-build-role-follows-the-deliverables-skill-vault-rows-skip-at-step-1]] — *added 2026-08-29:* the ADR that makes this rule structural in the sprint loop — vault-deliverable rows are skipped at step 1 rather than handed to a coder that must refuse
- [[tasks/decide-how-the-ship-loop-handles-a-non-coder-owned-task-row]] · [[tasks/record-that-0250-discharged-0188s-d1-and-warn-off-the-reordering]] — *added 2026-08-29:* `0270`, whose brief ⛔ **ruled this ADR out of scope on sight** — *"any candidate whose effect is a coder vault write is rejected"*; `0324`, which ⚠️ **neither wrote nor MEASURED the vault** and said so rather than reporting a zero
- *Added 2026-09-10 (sync `cf289c2`→`b4a1a52`):* [[tasks/build-the-link-resolution-guard]] — task `0354`, the link-resolution guard · [[tasks/sweep-b-the-single-site-correction-notes]] — task `0357`, Sweep B — the single-site correction notes · [[tasks/sweep-c-the-wiki-vault-resyncs-as-one-pass]] — task `0358`, Sweep C — the wiki-vault pass that advanced the watermark over a bounded-out backlog
- *Added 2026-09-16 (sync `b4a1a52`→`81f1429`):* [[decisions/adr-048-a-half-landed-close-gets-a-producer-only-reconcile-mode-that-never-upgrades-the-marker]] — its **must-never 9** carries this ADR's bar into the reconcile mode verbatim: ⛔ *never write `ai-agents/wiki-vault/`*
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/the-two-citation-sweeps-architecture-md-and-the-repo-wide-adr-line-class]] — task `0393`: ⭐ **`E6` ROUTED the vault's own citation sites to `fkit-wiki` rather than writing them** — 28 occurrences across 5 files, the rule working as designed
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/build-the-producer-only-sprint-movers]] — task `0341`: the sprint movers' repo-wide link sweep **excludes `ai-agents/wiki-vault/` by construction**
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/architecture-md-prose-repair-9-1-inventory-occurrence-b-and-9-5]] — task `0392`: ⛔ **vault sites were ROUTED to `fkit-wiki`, not written** — its `## Out of scope` names this rule by ADR number
- *Added 2026-09-16 (sync `b4a1a52`→`a351cb6`):* [[systems/backlog-convergence-and-the-k-measurement]] — ⭐ **this rule observed by a report that declined to file ITSELF.** Its closing section reads *"It is **not** filed to the wiki. Whether `ai-agents/wiki-vault/` should carry this is `fkit-wiki`'s call, not a producer's"* — ⭐ **a spawned producer reaching the vault boundary and stopping at it**, the same shape as `0393`'s `E6` routing and `0356`'s report-don't-repair fence
- *Added 2026-09-16 (sync `b4a1a52`→`a351cb6`):* [[tasks/correct-the-false-ci-has-never-run-claims-in-architecture-md]] — task `0312`: ⭐ **this rule honoured by a row that had a reason to break it.** Its constraint reads *"No `ai-agents/wiki-vault/` write"*, and its exact-phrase vault grep at filing found no copy of either passage — with the standing instruction that **if one had appeared, REPORT it, do not write it**
