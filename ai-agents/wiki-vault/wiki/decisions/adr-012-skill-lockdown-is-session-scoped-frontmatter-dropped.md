...
- [[tasks/add-adr-037s-worker-side-precedence-clause-to-the-universal-rules-block]] — task `0190`, sited in the rules block **precisely to escape this ADR's trap**: a rule written in a `SKILL.md` the worker never loads does not bind
- [[tasks/decide-whether-process-review-is-always-the-coder-or-the-architect-gains-the-skill]] — task `0200`, which declines to edit `skills_for_role()` — this ADR's *"only place role→skill ownership is expressed"*. ⚠️ **This ADR names the stale home `claude/fkit-claude.sh`; the function lives in `claude/skills-for-role.sh` — cite the file, not the ADR's path**
