...
- [[decisions/adr-042-a-codex-review-is-reasoning-only-and-reports-must-say-so]] — ⚠️ *Added 2026-08-13:* this ADR's §Context finding — Codex natively reads the project-root `AGENTS.md` because of `--cd`, so the shared-instructions layer genuinely reaches it — **is unchanged and still true.** What ADR-042 adds is the flag's *other* consequence: `--sandbox read-only` means the model receiving those rules **can reason about the diff and cannot measure anything.** ⚠️ **D2 (decided 2026-08-11, NOT built) would change the flag but not the `--cd`**, so the `AGENTS.md` reach described here survives either way
- [[tasks/verify-the-codex-half-of-the-comment-stripping-canary]] — ⚠️ *Added 2026-08-22:* task `0177`, the harness-version discipline applied — **`codex-cli 0.145.0` does not strip HTML comments from `AGENTS.md`**, version-stamped because a stripping behaviour is a harness detail that expires when either build moves
