...
- *Added 2026-09-16 (sync `b4a1a52`→`81f1429`):* [[decisions/adr-048-a-half-landed-close-gets-a-producer-only-reconcile-mode-that-never-upgrades-the-marker]] — ⭐ **this ADR's §Consequences is cited as the reason `/fkit-task-cancelled` gets NO mirror mode**: `cancelled/` is the board nobody audits, which makes it the worst place to open a first agent door
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/decide-the-sanctioned-repair-path-for-a-half-landed-close]] — task `0134`: ⭐ **its forgeability analysis is re-used again** — ADR-048's producer-only reconcile mode rests on ADR-033's rule, and ⚠️ **a SPAWNED producer qualifies, which is both the point and the risk**
