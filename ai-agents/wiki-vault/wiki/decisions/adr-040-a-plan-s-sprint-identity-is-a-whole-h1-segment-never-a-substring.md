...
- *Added 2026-09-16 (sync `b4a1a52`→`81f1429`):* [[decisions/adr-047-a-sprint-has-an-explicit-status-and-current-means-every-in-progress-sprint]] — depends on this ADR's identity grammar and **cannot ship before it**. ⭐ It also names the H1 as a **rejected carrier** for sprint status, on this ADR's own reasoning: *a status is not an identity*
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/pin-the-five-unpinned-behaviors-in-the-sprint-identity-grammar]] — task `0271`: ⛔ **§2.5's distinct-token refusal and §2.1's first-line narrowing both survived deletion with the suite green (129/129)** until this row pinned them
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/give-the-selector-a-status-rung-and-a-lowest-first-choice]] — task `0338`; ⭐ the line-3-only banner read follows this ADR's whole-segment posture
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/record-the-sprint-lifecycle-adr-047]] — task `0337`: ⭐ **the *"a WRONG identity is strictly worse than NO identity"* posture reused for status** — a bannerless plan resolves `unresolved` and emits a loud drift, never a silent `In progress`
