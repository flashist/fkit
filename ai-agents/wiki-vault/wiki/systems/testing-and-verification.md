...
> ⚠️ **SYNC 2026-09-16 — stale a FOURTH time, and the prediction has now held four for four.** ⛔ **Every count above is left byte-identical**; the live figures, measured on disk this run, are here.
> - **`ls test/*.test.js | wc -l` = 29** (20 → 21 → 25 → **29**). ⭐ **Only ONE of the four arrived inside this sync's window** — `mover-exemption-step` (`0381`, Sprint 8), which pins the mover clause across four skill copies; the other three landed between the 2026-08-29 lint and `b4a1a52` and are named here so the enumeration is complete: `coordination-citation-policy` (`0176`), `reference-integrity` (`0354`) and `throughput-counter` (`0359`), all Sprint 7.
> - **`test/prove-red.sh`'s header declares THIRTY-NINE mutations** (22 → 28 → 34 → **39**), and ⭐ **its own header carries the reason this page keeps having this problem:** *"KEEP THIS LIST IN STEP WHEN YOU ADD ONE — it read 'Two mutations' while seven more sat below it … in the one file whose entire thesis is that an unexercised gate hides drift."* **33–34 name `0381`; 35–39 name `0388`** — see [[tasks/give-the-sprint-mover-pins-and-successor-mode-durable-prove-red-mutations]].
> - ⭐ **`architecture.md` now solves the same problem a different way, and it is worth contrasting:** owner ruling 2026-09-14 (*"Count + named groups (Rec)"*) had §9.1 **stop enumerating suite names by hand**, keeping a **dated count plus the derivation command** and naming suites only by group. ⛔ **This page still enumerates, deliberately and with the risk stated above** — ⚠️ **but four consecutive stale counts is the evidence, and nothing here detects it either.**
>
...
- ⚠️ **`architecture.md` §9.1 is behind the code.** It still names *"zero automated verification"* as the top structural risk and states both high-blast-radius files are *"POSIX shell with no coverage of any kind."* **`claude/fkit-claude.sh` now has coverage** — argv contract + the lockdown matrix. **`install.sh` genuinely still has none**, so the risk is *reduced, not closed*. Flagged for the architect.
  ...
  > ✅⛔ **DATED CORRECTION 2026-09-16 (the `b4a1a52`→`c59f4d7` sync) — §9.1 IS NO LONGER BEHIND THE CODE, AND THIS BULLET'S FLAG IS DISCHARGED. The original is left byte-identical.**
  >
  > ⭐ **The flag was taken up and carried by a chain of rows, most of which never landed:** `0312` rewrote **occurrence A only**; `0251`, `0376` and `0366` were all **cancelled**; their scope was consolidated by owner ruling into `0392`, which shipped in Sprint 9 — [[tasks/architecture-md-prose-repair-9-1-inventory-occurrence-b-and-9-5]].
  >
  > **What §9.1 says now, re-read on disk this sync:**
  > - ⛔ **The *"zero automated verification"* framing is gone.** The heading reads *"The suite now runs automatically — CI plus an in-release gate; `install.sh` is still uncovered"*, and the live thesis is ⭐ **coverage, not automation**.
  > - ⭐ **The suite inventory is dated and derivable:** *"29 `node --test` suites, counted 2026-09-14"*, with *"re-count with `ls test/*.test.js | wc -l` rather than trust the figure"*. ✅ **Re-measured this sync: 29.** ⭐ **Suites are named BY GROUP and explicitly "not as a complete list"** — owner ruling 2026-09-14, verbatim *"Count + named groups (Rec)"*, taken because a hand-maintained list of filenames is precisely what rotted (the count moved **8 → 19 → 20 → 29**).
  > - ⭐ **CI's record is stated with its measurement date:** *"Measured **2026-09-14** … 43 runs on `ubuntu-latest`, 39 green and 4 red"*, with ⚠️ ***"Those are counts on a date, not a standing guarantee."*** ⛔ **The four falsified clauses — *never observed green*, *lands unpushed*, *only ever run on darwin*, and the dash risk stated as live — are all gone.**
  > - ✅ **The first red run is described correctly** as the **filesystem case-sensitivity** divergence repaired by [[tasks/make-the-lockdown-guard-case-test-filesystem-independent]] — ⛔ **not as the predicted dash failure.** ⚠️ *Getting that backwards would have replaced one false claim with another.*
  >
  > ⛔ **`install.sh` still has zero automated coverage, and §9.1 still says so.** ⭐ **This correction discharges the staleness flag, NOT the gap** — *"automating a suite that never touches it does not change that."* A `shellcheck` pass and a smoke install into a temp `$HOME` remain unwired.
...
- *Added 2026-09-10 (sync `cf289c2`→`b4a1a52`):* [[tasks/build-the-coordination-citation-policy-guard]] — task `0176`, the coordination-citation policy guard · [[tasks/build-the-link-resolution-guard]] — task `0354`, the link-resolution guard · [[tasks/settle-the-reference-integrity-condition-once-for-both-halves]] — task `0353`, the settled reference-integrity condition · [[tasks/sprint-7-stop-manufacturing-record-repair-rows]] — Sprint 7, the board that attacked the record-repair rate (archived 2026-09-08) · [[tasks/start-the-lead-session-not-the-producer-after-a-fresh-projects-cold-start]] — task `0379`, the cold-start lead routing
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/pin-the-five-unpinned-behaviors-in-the-sprint-identity-grammar]] — task `0271`: ⛔ **two correct behaviours measured to survive deletion with the whole suite green (129/129)**
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/give-the-task-movers-a-step-for-the-named-exempt-keys]] — task `0381`: the `mover-exemption-step` suite and `prove-red.sh` mutations 33–34
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/give-the-sprint-mover-pins-and-successor-mode-durable-prove-red-mutations]] — task `0388`: ⭐ **mutations 35–39**, and the rule that a one-off by-hand build measurement is **not a gate**
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/add-dual-home-paritys-missing-accepted-drift-row-and-correct-its-count]] — task `0389`: ⚠️ **a documentation-truth defect no test pins, and none will**
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/architecture-md-prose-repair-9-1-inventory-occurrence-b-and-9-5]] — task `0392`, which discharged this page's §9.1 staleness flag
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/sprint-9-settle-architecture-mds-truth-and-sweep-the-citation-rot]] — the board that carried all of the above
