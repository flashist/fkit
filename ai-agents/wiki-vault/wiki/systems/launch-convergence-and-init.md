...
- [[tasks/design-the-post-update-structure-check]] — task `0241` (2026-08-06): the owner-ruled design for a consent-gated structure check + repair beside this convergence pass — the unattended launch path's invariant stays unchanged; at most a read-only stderr notice is proposed for launch (owner-ruled follow-ups `0242`–`0249`, not yet built)
  - ✅ **Dated correction 2026-08-13 (the `0263` resync; the line above is left byte-identical). *"Not yet built"* is FALSE — all eight follow-ups shipped on [[tasks/sprint-4-ship-the-use-ready-self-healing-update]] and closed 2026-08-07/10**: the structure-spec, the hash manifest, the producer-owned check skill, the consent-gated repair path, the launch notice and its per-path intent-file suppression, the docs, and the vault ingest. **The stderr notice is no longer *proposed* — it shipped** (`0247`). ⚠️ **All eight closed `(agent-closed — not owner-verified)` and Sprint 4 was ARCHIVED UNVERIFIED by owner ruling — nothing this capability shipped has been exercised outside this repo.** The promise to verify is carried by task `0262`, which is unscheduled and has **not** run.
- [[tasks/build-the-producer-owned-structure-check-skill]] — `0245`: the check, **read-only in every branch**
- [[tasks/build-the-consent-gated-repair-path-inside-the-check-skill]] — `0246`: the repair, **in-session, consent-gated, never silent, never stored**
- [[tasks/add-the-launch-time-structure-notice-and-intent-file-suppression]] — `0247`: the awareness layer on this very pass; **per-path intent, never a cursor, never a global switch**
- [[tasks/author-the-structure-spec-and-its-scaffold-inventory-drift-test]] — `0243`: what should exist
- [[tasks/build-the-hash-manifest-generator-and-completeness-test]] — `0244`: **the only thing that can decide touched-or-not**
- [[tasks/correct-the-five-remaining-prose-sites-that-state-the-glob-as-the-mechanism]] — `0267`: ⚠️ a generated `backlog.md` header in a downstream project is **project content that launch convergence will never repair** — the same reachability gap this page's invariant creates by design
- [[decisions/adr-039-consent-gated-structure-repair-licensed-adr-015-invariant-unchanged]] — the companion ADR recorded 2026-08-07 (`0242`): licenses the in-session consent-gated repair while this launch path's invariant stays unchanged and in force; the launch path itself gains nothing beyond the read-only notice
- [[tasks/make-the-lockdown-guard-case-test-filesystem-independent]] — `0283`: the orphan-cleanup **lockdown-state guard** under test. ⚠️ The guard's delete-preventing skip is pure `tr` + glob with **no filesystem consulted**, so it behaves identically everywhere; only its **announcement** is existence-gated, deliberately. ***The failing test was really asserting "the filesystem folded case for me."***
...
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/sweep-the-repo-only-claude-path-form-out-of-installed-facing-prose]] — task `0390`: ⚠️ **each rewritten path had to be checked against `fkit-claude-init.sh`'s actual copy targets** — *"a confidently wrong path is worse than the inconsistent one"*
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/add-dual-home-paritys-missing-accepted-drift-row-and-correct-its-count]] — task `0389`, the `.fkit-accepted-drift` launch-notice intent file and why it is `live-only`
