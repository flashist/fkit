...
- **The task status vocabulary is a closed set**: `Backlog` · `In progress` · `Blocked` · `Done` · `Cancelled` · `Moved`. **No other value is valid** — not "Not started", not "WIP", not "Todo". `Done` and `Cancelled` are still set only via `/fkit-task-done` and `/fkit-task-cancelled` — but **they are no longer owner-only.** [[decisions/adr-025-spawned-agents-may-invoke-the-task-movers]] (2026-07-18) removed that gate: **any spawned agent may now run either mover**, and an agent-performed close-out is supposed to carry a distinct `(agent-closed — not owner-verified)` marker. **That marker is prose and unenforced** — read the ADR's honesty clause before trusting a green row. If a status you need isn't there, **amend the convention — don't invent a value inline.**
