...
>
> ⚠️ **LINT 2026-08-29 — item 3's run tally is STALE, and this is the THIRD time the vault has reported it so.** ⛔ **Item 3 is left byte-identical**; its `5 runs — 4 success, 1 failure` was correct on 2026-08-13 and is a dated measurement, not a defect. **Re-measured this run with `gh run list`: 24 runs — 22 success, 2 failure**, spanning 2026-08-12 → 2026-08-28. ⭐ **A SECOND red run exists** (2026-08-21) that no vault page had recorded; the first is still `0283`'s. ⛔ **Item 3's two load-bearing conclusions are unchanged and stronger, not weaker:** neither *"there is no CI"* nor *"CI is always green"* is true of this repo, and a red run remains a point in CI's favour. ⛔ **The tally is recorded here as a dated measurement and is deliberately NOT re-pinned into item 3** — a live run count on a living page is exactly what has gone stale three times, and the 2026-08-14 sweep already barred publishing a per-run set on this page. ⚠️ **Unverified: what the 2026-08-21 failure caught.** It was not investigated this run and no task is known to own it — stated rather than guessed.
...
> ✅ **Dated correction 2026-08-14 (the post-`0288` sync; EVERY block above is left byte-identical). `0288` HAS LANDED — the open-defect paragraph above is now HISTORY, and the "owes ONE MORE look" flag inside it is DISCHARGED by this note.** Task `0288` closed `✅ Done (agent-closed — not owner-verified)`; its folder moved to `ai-agents/tasks/done/0288-fix-the-post-release-verify-lines-failing-and-false-green-cases/`, committed in `9e61f9b`. ⛔ **The path and `🔲 Backlog` status quoted above are stale by design — they were true when written.** Full record: [[tasks/fix-the-post-release-verify-lines-failing-and-false-green-cases]].
>
> **What the release summary does now.** It describes **the run that happened**, derived from the **measured** pre-run tag state rather than from `--no-tag` / `--no-push` alone — a distinction that is load-bearing, because both flags are **true** in the local-only-tag state, so a guard on the flags alone would have fixed one defect and left another untouched. The default path's headline and success behaviour are **byte-identical**; a **speaking failure branch** was appended to the printed command, on the owner's ruling *"(A) Fix R5 on the default path too"*. `bin/release.mjs` is now **385 lines** — ⛔ **every `:NNN` written about that file on this page, above, is stale.**
>
> ⛔⛔ **It is NOT "the release path is fixed". FIVE owner-ruled residuals ship with it**, and a reader who drops them has the wrong picture:
>
> 1. **`--no-bump` still cannot finish a release whose tag is local-only** — it **stops lying about it** and prints the by-hand `git push origin <tag>`. ⛔ Not *"recovery restored"*; the fence is asserted by a test.
> 2. **The `push.followTags=true` case is open BY DESIGN** (owner: *"Leave it documented"*) — the script still cannot see a tag its **own branch push** published, so the UNFINISHED branch can say `NOT pushed` of a tag that was. The **fixture** is pinned so the suite passes anywhere; the **script** is not fixed.
> 3. **Three test-rigour residuals owner-ruled not actioned** — mutation isolation is documented rather than gated, two assertions have no mutation, and mutation 22 has no wrong-target guard.
> 4. **`bin/release.mjs:12-14` still calls a `--no-bump` run *"idempotent"***, which overstates it: skipping tag *creation* is idempotent, skipping the tag *push* is not.
> 5. **The unquoted `${tag}` is untouched** (owner-ruled *"Unactioned — pre-existing"*), **Windows shells are uncovered**, and a global **`core.hooksPath`** with a rejecting `pre-commit` still reds the whole new suite — a known, pre-existing host dependency.
>
> ⚠️⚠️ **A SEPARATE release-path defect is now open — task `0300`, above the summary block and untouched by `0288`.** `release.mjs --branch <other>` **commits and tags HEAD** but pushes a **different** ref, then prints `✓ Released`. **`0288`'s own verify command PASSES on it** — the tag really is on origin, it just names a commit **reachable from no branch on origin**; under `--no-push` the printed recovery command hand-builds that state. ⚠️ **Found by the coder during `0288`'s review and raised by NEITHER reviewer**, reproduced first-hand at filing. Backlog, `🔲 Backlog`, **unranked**, open.
>
> ⚠️ **On runtime:** `0288` increased the suite's cost and the owner accepted a measured **~+40%** (verbatim *"Accept the +40%"*). ⛔ **No measured absolute figure is recorded here** — three exist for the same code and every worker that reported one said it was moving with **machine load**; a clean idle measurement is still owed. The standing authority remains `RELEASING.md:128`'s ruled *"roughly 6–8 minutes, machine-dependent"*, ⚠️ **which was ruled before `0288` added its cost and may now understate.** ⛔ **Reported, not fixed** — `RELEASING.md` is outside this vault, and re-ruling the figure is the owner's act.
...
- **The agent count is still a hard-coded literal, not derived.** `fkit-claude-init.sh` prints `Seven roles` as a `printf` string, while the line that actually *counts* the copied agents (`n_agents`) is separate. **The two can drift apart again silently** — they have before. *(Re-verified 2026-07-16: the string reads "Seven" and is correct, and the stale `fkit claude` usage comment is gone. `architecture.md` §9.5 still lists both as open drift — **the doc is behind the code**.)*
  ...
  > ⛔⛔ **DATED CORRECTION 2026-09-16 (the `b4a1a52`→`c59f4d7` sync) — THE BULLET ABOVE IS FALSE IN BOTH HALVES. The original is left byte-identical.**
  >
  > 1. ⛔ **There is no `Seven roles` literal any more.** **Re-measured on disk this sync:** the summary block prints **NO role count at all**, and the line carries a comment recording that as **deliberate, owner-ruled 2026-07-20 (`0036` Part D)** — *"NO ROLE COUNT ON THIS LINE"*, noting that it *"used to read 'Seven roles, …'"* and that ADR-028 adds an eighth. ⭐ **So the drift hazard this bullet named was removed by deleting the literal, not by keeping it in step.** The derived `n_agents` value survives on a different line and prints as *"refreshed N agents"* — ⭐ **it counts refreshed agents, not roles.**
  > 2. ⛔ **`architecture.md` §9.5 no longer lists either item as open drift, so *"the doc is behind the code"* is spent.** §9.5 was rewritten by `0392` and now reads, in one dated sentence, that *"the residuals previously listed here were verified discharged or false on 2026-09-14"* — naming the init summary's role count, the `fkit claude` usage comment, and the Omnigent ADRs' status. See [[tasks/architecture-md-prose-repair-9-1-inventory-occurrence-b-and-9-5]].
  > 3. ⚠️ **`0366`'s own correction of this claim was ITSELF half wrong and must not be inherited** — it described the script as printing *"a **derived** count from `ls … | wc -l`"*. ⛔ **It prints no role count at all.** ⭐ *That is why `0392`'s Group C said "firsthand", not "confirm".*
...
- [[tasks/sprint-5-fix-what-a-real-project-found]] — the release-hygiene cluster's board. ⚠️ *Corrected 2026-08-14:* this line read *"the **live** board"*; it is **archived**, at `ai-agents/sprints/done/sprint-5.md`, by task `0294`. ⚠️ **There is no active board at all right now** — `dashboard.sh select-active ai-agents/sprints` returns `active none`
  ...
  > > ⚠️ **Further dated note 2026-08-29 — the 2026-08-22 note above is itself now dated; both lines are left byte-identical.** Sprint 6 has since been **archived** to `ai-agents/sprints/done/sprint-6.md` (banner `## 🔒 CLOSED — 2026-08-29.`), so the path it names is the pre-archival one and *"is the active board"* was true of 2026-08-22, not of today. ⛔ **What the current active board is, is deliberately NOT asserted here** — it was in flux when this note was written, and a standing claim about it is exactly the shape that has gone stale on this page twice.
...
- [[tasks/fix-the-post-release-verify-lines-failing-and-false-green-cases]] — ⚠️ *Added 2026-08-14:* task `0288`, which rewrote `bin/release.mjs`'s post-release summary and **closed the open-defect paragraph above** — shipping **five owner-ruled residuals** and surfacing task `0300`, still open
- [[tasks/scope-readme-54s-fkit-managed-structure-sentence-to-what-the-check-covers]] — ⚠️ *Added 2026-08-14:* task `0292`, which rescoped the launch notice's README description to `ai-agents/` **plus** the two root context files, and states why `.claude/` is outside the check
- [[tasks/correct-the-superseded-runtime-figure-on-the-gate-releases-page]] — ⚠️ *Back-link added by the 2026-08-14 lint:* task `0297`, which corrected the superseded suite-runtime figure and the barred four-run duration list on [[tasks/gate-releases-so-an-untested-tree-cannot-ship]], and recorded its unfixed residuals — one of them against this page's dated `0289` block. ⛔ **Its standing bar carries here too: cite `.github/workflows/test.yml` and `0252`'s ledger by anchor, never re-publish a per-run duration list**
- [[tasks/release-mjs-with-branch-other-commits-and-tags-head-but-pushes-a-different-ref]] — *added 2026-08-29:* `0300`, which closed a **false green** in `bin/release.mjs` — `--branch <other>` announced `✓ Released` while publishing a tag no origin branch reached
- *Added 2026-09-10 (sync `cf289c2`→`b4a1a52`):* [[tasks/cut-the-v0-3-0-release-and-hand-archive-sprint-7]] — task `0360`, the v0.3.0 cut and Sprint 7's hand-archive · [[tasks/start-the-lead-session-not-the-producer-after-a-fresh-projects-cold-start]] — task `0379`, the cold-start lead routing
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/sweep-the-repo-only-claude-path-form-out-of-installed-facing-prose]] — task `0390`: ⛔ **a consuming project has only `.claude/`**, and eight runnable command lines in shipped skills used the repo-only form — including in skills that call a script whose own banner declares the installed form
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/architecture-md-prose-repair-9-1-inventory-occurrence-b-and-9-5]] — task `0392`, which falsified this page's `Seven roles` bullet (see the dated correction above)
