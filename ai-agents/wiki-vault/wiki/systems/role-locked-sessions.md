...
- [[tasks/decide-whether-process-review-is-always-the-coder-or-the-architect-gains-the-skill]] — task `0200`, the first ruling **on the invocation axis this page owns** since ADR-018: a driver spawned `@fkit-architect` for a **coder-owned** step on three consecutive tasks, **the hook denied it correctly**, and the ruling is **route the step to its owning role, do not widen `skills_for_role()`**. ⚠️ **Its honest limit belongs on this page:** the hook gates skill **invocation**, so **a driver that spawns the wrong role and tells it to work by hand never reaches the gate** — the fix is prose plus a detector, **not prevention**
- [[tasks/add-adr-037s-worker-side-precedence-clause-to-the-universal-rules-block]] — task `0190`, the content-axis rule delivered on the one surface that reaches every spawned worker — **the site chosen precisely because a rule in an unloaded `SKILL.md` does not bind**
...
- [[tasks/add-adr-037s-driver-side-clause-to-the-sprint-ship-loops-hard-rules]] — task `0191` — ⚠️ **the canonical-vs-runtime gap this page depends on, seen from the other side:** an edit to `claude/skills/**` binds nobody until `fkit-claude-init.sh` refreshes the **gitignored `.claude/` copy a session actually loads**. Measured at 296 lines with the clause absent, a day after it shipped
- [[tasks/build-the-producer-owned-structure-check-skill]] — task `0245` (2026-08-07): a new producer-owned skill, wired into `skills_for_role()` **and the hook's test matrix in the same act** — the `0112` lesson applied, since an unwired skill exists and the hook denies it to everyone
- [[tasks/repair-the-five-live-ownership-fact-defects]] — *added 2026-08-29:* `0188`, which repaired five live ownership-fact defects across `PROJECT.md`, `architecture.md`, root `CLAUDE.md`, the scaffold and the mirror checklist — including the **FOUR**-mirrors count that was off by at least two, in two byte-identical copies neither of which points at the other
