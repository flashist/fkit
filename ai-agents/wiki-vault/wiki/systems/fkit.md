...
> ⚠️ **LINT 2026-08-13 — the trailing clause is FALSE and the sentence is left byte-identical.** ***"no test suite" has not been true since Sprint 2.*** Measured on disk this run: **20 `test/*.test.js` files plus `test/prove-red.sh`**, run by `npm test` — and since 2026-08-12 **run automatically by `.github/workflows/test.yml`** on every push to `main` and every pull request ([[tasks/gate-releases-so-an-untested-tree-cannot-ship]]). ✅ **The rest of the sentence is correct and unaffected** — no build step, no server, no database, no runtime state outside files. *(This claim predates the 2026-08-13 sync; it was not introduced by it.)*
...
> ⚠️ **SYNC 2026-09-16 (`b4a1a52`→`c59f4d7`) — THE COUNT IS NOW 28. Every figure below is left byte-identical.**
> ⭐ **Measured on disk this run: `ls -d claude/skills/fkit-*/` = 28** (25 → 26 → **28**). The two new
> ones are the **sprint movers** — `fkit-sprint-done` and `fkit-sprint-cancelled` — built by `0341` on
> owner ruling SD-3 of 2026-08-25, verbatim *"Mover skills, producer-only (Recommended)"*. See
> [[tasks/build-the-producer-only-sprint-movers]].
> ⛔ **They are PRODUCER-ONLY and were so from their first line** — `claude/skills-for-role.sh`'s own
> header records that they *"were never granted to anyone else"*. ⭐ **The `MOVERS` invariant in
...
> - ⛔ ***"naming the one role allowed to run it"* — false for two skills.** `open-questions-interview` and `dumb-down` name **the six Claude-side roles**, not one; the architecture doc now reads *"the role (or roles) allowed to execute it"*. The table below already carries that `everyone`/six-role row correctly.
> - ⛔ ***"Only `fkit-query` carries no banner"* — false: `fkit-team` carries none either.** Re-measured under `0188`: **24 of 26** skills carry a `⛔ Owner:` banner; the two without are the **two universal skills**, `fkit-query` and `fkit-team`. ⭐ The defect survived a `grep -q "⛔ Owner:"` audit **because `fkit-team`'s prose *describes* the banner, and prose about a marker satisfied the grep for the marker** — caught by opening the file, not by refining the pattern.
>
> ⚠️ **LINT 2026-08-13 — the heading and the count above are STALE by one; both left byte-identical.** **Measured on disk this run: 26 skill dirs.** The addition is **`fkit-heal`**, the producer-owned structure-conformance check and its consent-gated repair, landed by [[tasks/build-the-producer-owned-structure-check-skill]] (`0245`) and [[tasks/build-the-consent-gated-repair-path-inside-the-check-skill]] (`0246`), and **declared to the producer in `skills_for_role()`** — verified. **It is missing from the ownership table below**; read the table as 25 of 26 until a future pass adds the row. ⚠️ *A hard-coded census in a heading goes stale on the next skill and nothing checks it — the same class this page records elsewhere about the agent-count literal, which was removed for exactly that reason.*
...
| producer | `initiate-project`, `task-brief` *(renamed from `task-plan`, [[tasks/rename-task-plan-skill-to-task-brief]])*, `task-done`, `task-cancelled` *(**producer-only again** since ADR-033)*, `status`, **`heal`** *(added 2026-08-07 — the structure-conformance check + consent-gated repair; ⚠️ **any repair under `ai-agents/wiki-vault/` is routed to `fkit-wiki`, never performed by the producer**, per [[decisions/adr-005-vendor-wiki-query-skill-reads-decentralized]])* |
...
- **Verification is partial, no longer absent.** *(Updated 2026-07-16.)* For most of fkit's life there was **zero** automated verification — the only check, `omnigent/validate-bundles.sh`, **died with the Omnigent removal**, and [[decisions/adr-003-ci-runs-validate-bundles]]'s CI never landed. **`claude/fkit-claude.sh` is now covered** by the launcher-contract suite (`npm test`) — the argv contract plus the hook-wiring shape; the **per-role skill matrix moved to `test/skill-ownership-hook.test.js`** when task 43 retired the session-scoped 7×21 `skillOverrides` matrix ([[decisions/adr-014-how-fkit-tests-itself]], [[decisions/adr-018-pretooluse-skill-ownership-hook-replaces-consult-skills-exception-list]], [[systems/testing-and-verification]]). **`install.sh` still has none, and there is still no `.github/`** — so the `curl | sh` entry point, the highest-blast-radius file in the repo, remains unverified. **The risk is reduced, not closed.**
  - ✅ **Dated correction 2026-08-13 (the `0282` resync; the sentence above is left byte-identical). Half of it is now false and half is still true, and the halves must not be collapsed.** ⚠️ **This reverses the owner's earlier `"No CI planned."` ruling:** `.github/workflows/test.yml` landed with [[tasks/gate-releases-so-an-untested-tree-cannot-ship]] (`0256`, 2026-08-12) and runs `npm test` on every push to `main` and every pull request, alongside a blocking `npm test` gate inside `bin/release.mjs`. ✅ **So "there is still no `.github/`" is FALSE.** ⛔ **"`install.sh` still has none" is STILL TRUE** — `install.sh` coverage and `shellcheck` were **explicitly out of `0256`'s scope**, as their own briefs. **The `curl | sh` entry point remains unverified; the risk is still reduced, not closed.** See [[systems/testing-and-verification]] §"CI and the release gate".
...
- ⚠️ **The task movers are PRODUCER-ONLY — reversed twice, currently back to one role** *(ADR-025 opened them 2026-07-18; [[decisions/adr-033-task-movers-are-producer-only-reversing-adr-025]] closed them again 2026-07-23)*. **Only `fkit-producer` may run `/fkit-task-done` and `/fkit-task-cancelled`.** Every other role — wiki, coder, reviewer, architect, lead — **routes its closes through the producer** and closes nothing itself. Verified against the tree 2026-07-26: `claude/skills-for-role.sh` grants both movers to `producer` and to no other role.
  > ⭐ **SYNC 2026-09-16 — THERE ARE NOW FOUR MOVERS, NOT TWO, AND SPRINT PLANS ARE GOVERNED THE SAME WAY.** The bullet above is left byte-identical and its rule is **unchanged and extended, not amended**. `/fkit-sprint-done` and `/fkit-sprint-cancelled` joined the pair in Sprint 8 ([[tasks/build-the-producer-only-sprint-movers]]), **producer-only from their first line**, hook-enforced by the same `skills_for_role()` mapping. ⛔ **A sprint plan now moves between `sprints/`, `sprints/done/` and `sprints/cancelled/` ONLY through them**, and a close with no owner present carries the same `(agent-closed — not owner-verified)` marker. ⭐ **Sprint 8 was the first board in this project's history closed by a mover instead of by hand** — [[tasks/sprint-8-give-sprints-the-lifecycle-tasks-already-have]]; Sprint 9 followed — [[tasks/sprint-9-settle-architecture-mds-truth-and-sweep-the-citation-rot]].
...
- [[decisions/adr-037-a-skill-rule-binds-a-spawned-worker-unless-the-instruction-relays-an-owner-ruling]] — **ADR-037** — a skill rule binds a spawned worker unless the instruction relays a named owner ruling; the precedence ladder, complete, for the first time. ✅ **The worker-side clause is now IN the universal rules block** (`0190`, 2026-08-04), so it reaches every spawned worker of every role on every turn — ⚠️ **the driver-side clause reaches no driver** (`0191`)
- [[tasks/decide-whether-a-spawn-instruction-may-override-a-skill-rule]] — task `0158`, the investigation behind ADR-037
- [[tasks/add-adr-037s-worker-side-precedence-clause-to-the-universal-rules-block]] — task `0190` — ⚠️ **the rules-block cap is `RULES_MAX=4352`, not 4096**, on an owner-signed ADR-016 bump; **every turn in every consuming project pays the extra bytes**
- [[tasks/add-adr-037s-driver-side-clause-to-the-sprint-ship-loops-hard-rules]] — task `0191` — ⚠️ **canonical-vs-runtime gap, worth generalizing:** an edit to `claude/skills/**` binds nobody until `fkit-claude-init.sh` refreshes the gitignored `.claude/` copy the session actually loads
- [[tasks/decide-whether-process-review-is-always-the-coder-or-the-architect-gains-the-skill]] — task `0200` — **a loop step's role is fixed by the skill the step runs, not by the deliverable's author**; the `route-don't-widen` pattern reaffirmed; the ADR it awaited (`0222`) was recorded 2026-08-06 as ADR-038
- [[tasks/decide-what-the-sprint-driver-does-when-a-spawned-worker-dies]] — task `0167` — what the driver does when a spawned worker returns nothing: **read disk, classify into three states, escalate on partial**
...
- [[tasks/amend-the-sprint-loops-honor-the-adrs-rule-with-the-faithful-carry-construction]] — task `0203` — ***a faithful carry is a copy operation over a durable artifact, never a recall over conversation state***, now written into the loop with a both-legs presence check
- [[tasks/add-an-exit-table-row-for-a-failed-build-verify-review-spawn-in-the-sprint-loop]] — task `0208` — the exit row for a spawn that does not land, and the orphaned-`plan.md` rule moved out of a task worklog **no future driver reads**
- [[tasks/sprint-3-close-the-rank-integrity-loop]] — ⚠️ *Corrected 2026-08-14:* this read *"the active board since the 2026-08-06 rollover"*. Sprint 3 was **archived 2026-08-07** at `ai-agents/sprints/done/sprint-3.md`; Sprint 2 is archived at `ai-agents/sprints/done/sprint-2.md`
- [[tasks/decide-whether-sprint-2-rolls-over-to-a-fresh-board]] — task `0185` — the owner-ruled rollover itself: board archived, 341 + 12 links re-pointed, fresh board at `P1`, no closed rank renumbered
- [[tasks/narrow-the-re-rank-exception-an-insertion-is-not-a-re-rank]] — task `0181` — ADR-035's narrowing written into `/fkit-task-brief` step 5: an insertion is never a re-rank
- [[tasks/build-the-closed-rank-immutability-guard]] — task `0182` — `test/closed-rank-immutability.test.js`, the board-history guard (⛔ not continuous protection: no CI runs it). ✅ *Corrected 2026-08-13: CI **does** now run it (`0256`), but the `⛔ not continuous` verdict survives on a narrower reading — its leg 2 sees `HEAD`↔`HEAD^` only, so a multi-commit push still hides a middle-commit breach. See [[systems/testing-and-verification]].*
- [[tasks/gate-releases-so-an-untested-tree-cannot-ship]] — task `0256` (2026-08-12) → **CI exists** (`.github/workflows/test.yml`) **and** a blocking `npm test` gate in `bin/release.mjs`. ⚠️ **Reverses the owner's earlier "No CI planned." ruling.** Neither gate replaces the other: installs track `main`'s HEAD, not a tag, and **CI never sees the working tree `git add -A` ships**
- [[tasks/make-the-lockdown-guard-case-test-filesystem-independent]] — task `0283` (2026-08-13) → fkit's **first-ever CI run went red** (708/709) and found a real **test** defect: an assertion that was really asserting *"the filesystem folded case for me"*, invisible on macOS
- [[tasks/sprint-4-ship-the-use-ready-self-healing-update]] — the structure-check and consent-gated self-heal board; **archived unverified**
- [[tasks/sprint-5-fix-what-a-real-project-found]] — 🔒 **CLOSED and archived** at `ai-agents/sprints/done/sprint-5.md`: the first defect a real downstream project reported in shipped fkit code, plus release hygiene. ⚠️ *Corrected 2026-08-14:* this read *"🟢 **the ACTIVE board**"*; task `0294` archived it. ⛔ **There is no active board at all right now** — `bash claude/skills/fkit-status/dashboard.sh select-active ai-agents/sprints` returns **`active none`**, with `backlog.md` (identity `Backlog`) the only candidate and `Backlog` never eligible. **All 17 rows keep `(agent-closed — not owner-verified)`; archival verifies nothing**
  > ⚠️ **Dated correction 2026-08-22 — the *"no active board at all right now"* clause above is FALSE as of 2026-08-14; the line is left byte-identical.** [[tasks/sprint-6-repair-the-record-the-board-rests-on]] opened that day and is the active board, 19 rows, ranks restarting at `P1`. **`active none` was true only for the gap between Sprint 5's archival and Sprint 6's opening.** ⛔ The rest of the line — the 17 agent-closed rows, and archival verifying nothing — is unaffected.
  > > ⚠️ **Further dated note 2026-08-29 — the 2026-08-22 note above is itself now dated; both lines are left byte-identical.** Sprint 6 has since been **archived** to `ai-agents/sprints/done/sprint-6.md`, so *"is the active board, 19 rows"* describes 2026-08-22 — it finished at **21 rows, all closed**. ⛔ *"Archival verifies nothing"* holds for Sprint 6 too, and doubly: its archival carries **no owner ruling at all**.
- [[tasks/sprint-6-repair-the-record-the-board-rests-on]] — ⚠️ *Added 2026-08-22:* 🟢 **the ACTIVE board**, opened 2026-08-14 on a triage of all 108 then-open rows. ~~**7 of 19 rows closed, all agent-closed.**~~ ⚠️ *Corrected 2026-08-29 (lint) — the count is left readable as the 2026-08-22 record:* **the board is now 21 rows, all 21 closed**, and ⛔ **it is still the ACTIVE board because nothing archived it** — no `🔒 CLOSED` banner, still at `ai-agents/sprints/sprint-6.md`, still returned by `select-active`, measured this run. ⚠️ It opened **unranked** — the first board in this project ever to — because the producer that opened it had no owner channel, and was ranked the same day by owner ruling
  > ✅ **Dated correction 2026-08-29 (later the same day, after the lint above) — the *"still the ACTIVE board because nothing archived it"* clause is now FALSE; the line is left byte-identical.** Something archived it: the board moved to `ai-agents/sprints/done/sprint-6.md`, gained a `## 🔒 CLOSED — 2026-08-29.` banner, and `select-active` **no longer returns it** — re-measured this run. ⭐ **The lint's finding was correct when made and was discharged, not withdrawn.** ⚠️ **This archival is the first of the six with no owner ruling behind it** — the archiving producer had no owner channel (ADR-021), so read it as agent-performed and **not owner-verified**; ⛔ all 21 row closes stay `(agent-closed — not owner-verified)`.
- [[tasks/pressing-enter-at-the-role-menu-should-open-the-lead]] — task `0302` (2026-08-21): pressing **Enter** at the role menu now opens the **lead**, making the interactive path agree with the headless default
- [[tasks/rewrite-the-false-no-ci-paragraph-and-fabricated-citation-in-fkit-wiki-lint]] — task `0280` (2026-08-15): a **false "no CI" claim and a fabricated citation** removed from the wiki role's own shipped lint procedure
- [[tasks/verify-the-codex-half-of-the-comment-stripping-canary]] — task `0177` (2026-08-16): **`codex-cli 0.145.0` does NOT strip HTML comments from `AGENTS.md`** — the rules-block wrapper is free on the Claude side and **paid in full on the codex side**
- [[decisions/adr-040-a-plan-s-sprint-identity-is-a-whole-h1-segment-never-a-substring]] — *a wrong identity is strictly worse than no identity*; **prose containment is not identity**
- [[decisions/adr-041-the-active-sprint-is-selected-by-resolved-identity-not-by-filename-glob]] — the `sprint-*.md` glob **retired**; `Backlog` is never eligible
- [[decisions/adr-042-a-codex-review-is-reasoning-only-and-reports-must-say-so]] — what the Codex second opinion guarantees, **bounded**: reasoning, not measurement
- [[tasks/record-adr-038-a-loop-steps-role-is-fixed-by-the-skill-the-step-runs]] — task `0222` — ADR-038 recorded: **a loop step's role is fixed by the skill the step runs**
- [[tasks/design-the-post-update-structure-check]] — task `0241` — the owner-ruled post-update structure-check design, and the sanctioned ADR-015 re-raise
