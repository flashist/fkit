...
- *Added 2026-09-16 (sync `b4a1a52`→`81f1429`):* [[decisions/adr-047-a-sprint-has-an-explicit-status-and-current-means-every-in-progress-sprint]] — ⭐ **this board is the named failure the ADR was written to close**: Sprint 5 stayed "active" while finished, because folder location alone cannot express `Backlog` vs `In progress`
