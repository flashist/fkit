...
- [[tasks/decide-what-the-sprint-driver-does-when-a-spawned-worker-dies]] — task `0167`, bound by this ruling and **its strongest corroboration**: eight of its ten inherited coordinates had shifted, and **the two that held were both into a frozen ledger under `done/`** — exactly the split predicted here. ⚠️ It also found **row 1 of this task's table unresolved** for a design doc citing a skill file, and asks the convention page to resolve it rather than copy it
- [[tasks/add-adr-037s-driver-side-clause-to-the-sprint-ship-loops-hard-rules]] — task `0191`, which shipped its clause with **no `path:NNN` coordinate**, verified by grep over the inserted lines
- [[tasks/repair-the-three-decay-shapes-across-the-open-backlog-briefs]] — ⚠️ *Added 2026-08-22:* task `0306`, the corpus-wide repair that landed under this ruling. ⛔ **It removed the day's seed, not the class** — *"the decay can return tomorrow"*
- ✅ *Added 2026-08-22:* **follow-up 1's convention page LANDED 2026-08-21** — `ai-agents/knowledge-base/conventions/durable-citation-anchors.md`, dual-homed byte-identically. ⚠️ ~~**Its task `0171` is still `🔄 In progress`**~~ ✅ *Corrected later the same day, 2026-08-22:* **`0171` CLOSED** — [[tasks/write-the-durable-citation-anchors-convention-page]], `✅ Done (agent-closed — not owner-verified)`, ⛔ **no human has verified it**. Still `P2` on [[tasks/sprint-6-repair-the-record-the-board-rests-on]]. See [[systems/knowledge-base-structure]]
- [[tasks/write-the-durable-citation-anchors-convention-page]] — task `0171`, **follow-up 1** — the page this report deliberately did not write. ⭐ **It also proved the form on its own subject matter:** repairing the 12 displaced `adr-010:NNN` pointers displaced **10 inbound `adr-012:NNN` pointers**, three of them naked — the identical mechanism one document over — and the strip-`:NNN` repair was **measured to terminate at wave 2**. ⚠️ Its own measurement corrected a claim this arc would have accepted: the ten pointers were **defective at authoring**, not broken by the repair
- [[tasks/remediate-the-dead-brief-paths-in-closed-review-ledger-headers]] — *added 2026-08-29:* `0168`, the execution arm of this decision's **Case 3**
- *Added 2026-09-10 (sync `cf289c2`→`b4a1a52`):* [[tasks/clean-the-coordination-citation-residual-set-that-blocks-0176]] — task `0237`, the citation-residual cleanup
- *Added 2026-09-16 (sync `b4a1a52`→`c59f4d7`):* [[tasks/the-two-citation-sweeps-architecture-md-and-the-repo-wide-adr-line-class]] — task `0393`: ⭐ **its fourth triage class, `mentioned`, is the case this decision did not have to face** — a coordinate the sentence is *talking about*, where repairing it corrupts the record
