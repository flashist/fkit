...
✅ **Dated clarification 2026-08-02 — the total is FOUR SITES, not five, and item 4's wording above is where the miscount came from.** The bullets above are left byte-identical; this note is placed at the claim. **`0124`'s final inventory is four sites: the universal rules block (item 1) plus THREE agent system prompts — `fkit-producer.md`, `fkit-coder.md` (item 2) and `fkit-lead.md` (item 4).** Item 3 names no new file; it is a second defect class in two files item 2 already lists. **Item 4's *"a fourth system prompt"* counts the rules block among the system prompts** — a defensible reading, since the block sits in every agent's context every turn, but it is why [[systems/fkit]] carried *"four system prompts **and** the universal rules block"* (five) until 2026-08-02, **double-counting item 1**. [[tasks/investigate-the-skill-ownership-fact-inventory-gap]] (task `0142`, report Part 7) recorded the discrepancy against the brief's and sprint plan's *"three"* and **could not settle it**; it is settled here from `0124`'s own amendment history — **"three" is the count after the FIRST amendment**, before `fkit-lead.md` was added by the third. ⚠️ **Residual:** `0124`'s folder has **no `worklog.md`**, so if its sweep found a site it never wrote into the brief, no record of it survives.
