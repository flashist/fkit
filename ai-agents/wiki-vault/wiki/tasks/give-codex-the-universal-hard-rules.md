...
- [[decisions/adr-042-a-codex-review-is-reasoning-only-and-reports-must-say-so]] — ⚠️ *Added 2026-08-13:* this page's reasoning about `--sandbox read-only` is **accurate today and load-bearing for two things it did not anticipate.** First, ADR-042 names what that flag actually costs: **Codex cannot run the test suite, build a fixture, or execute a mutation — so the second opinion is *reasoning*, never *measurement***, and reports must say so. Second, ADR-042's **D2 (decided 2026-08-11, NOT built — task `0273` open)** would move all four executable sites to `workspace-write`, at which point *"it cannot move a file anyway"* — the premise this page's rule-3 wording rests on — **stops being true.** Re-read rule 3's wording when `0273` lands.
- [[tasks/verify-the-codex-half-of-the-comment-stripping-canary]] — ⚠️ *Added 2026-08-22:* task `0177` — measured that codex **pays in full** for the block's HTML-comment wrapper, unlike Claude
