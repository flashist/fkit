...
### The two investigations that closed with "build nothing" (46, 49)
46 → [[decisions/adr-026-no-mutation-testing-library-prove-red-stays-hand-rolled]]: **no mutation-testing library can mutate shell**, so ADR-014's zero-devDeps principle was never actually in tension — there was no contender. The real defect was **gating**: `prove-red.sh` runs only when a human types it. 49 → [[decisions/adr-027-dual-home-parity-is-a-dev-time-convention-plus-test]]: a convention plus a parity test for the **six** drifted dual-homed files, with the separate consuming-project drift decision **deliberately left deferred despite its re-raise trigger having fired.**
...
**Still Backlog (11):** **45 / 51 / 66 / 69 / 71 / 73** — six pre-filed wiki syncs, of which **45, 51, 66, 69, 71 and 73 are all substantively covered by the 2026-07-19 sync** that recorded this batch; **58** the ADR-022 doc refresh; **63 / 64** the spawned-mover work (63's design is done and recorded as ADR-025 though its row still reads Backlog — see below; 64 is gated on a mandatory adversarial pass).

⚠️ **Board-vs-record drift worth flagging:** **task 63 is still marked `🔲 Backlog`** on the board, but its design was completed and ruled on — [[decisions/adr-025-spawned-agents-may-invoke-the-task-movers]] exists, dated 2026-07-18, and cites task 63's report as its evidence. Similarly **task 46 and task 49 read Backlog** while ADR-026 and ADR-027 record their completed investigations. The wiki role does not move task files; the owner may want to close them.
