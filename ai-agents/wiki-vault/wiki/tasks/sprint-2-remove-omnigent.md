...
>
> Sprint 2 shipped **8** tasks in the run that performed the cleanup — `0143`, `0158`, `0162`, `0195`, `0202` (closed 2026-08-02), then `0173`, `0210`, `0211`. Every one is `(agent-closed — not owner-verified)`.
>
> ⚠️ **Dated correction 2026-08-03 (lint) — that count is SEVEN for this board, not eight; the sentence above is left byte-identical.** `0211` has **no row on `sprint-2.md` at all** — its row sits on the unranked **Backlog** board, as [[tasks/annotate-the-old-form-completion-flags-in-the-vault-log]] and this vault's `index.md` both state. The run closed eight tasks; **seven of them were Sprint 2 rows.** This board's own Done cells moved `130 → 132` on 2026-08-03, which is the arithmetic that settles it. Everything else in the sentence — the eight IDs and the universal agent-closed marker — is correct.
...
### The wiki-flag repair and the 45-row de-scope (0173 · 0210 · 0211, closed 2026-08-03)
- [[tasks/tighten-the-wiki-completion-flag-block]] — task `0173` → the wiki completion flag becomes **folder-ID-only, no path**, plus a `:NNN` ban. ⚠️ **Its own fix created a fresh instance of the class it removed** — the routing line's `<brief path>` placeholder went unbound, caught in review and repaired with a glob lookup. ⚠️ **The board's ranks contradicted its dependency links**, and the links were the binding record
- [[tasks/specify-and-support-the-reverse-move-sprint-to-backlog]] — task `0210` → the reverse move specified at last: marker `➡️ Moved to [Backlog](backlog.md)`, **five** mandatory edits (not the forward form's three), rank surrendered, and a `sed -nE` parser fix. ⚠️ **The obvious BRE fix silently does nothing on BSD `sed`** — passes Linux CI, fails on the owner's Mac, no signal either way. ⚠️ Its review **refuted the plan's own justification** for skipping prove-red coverage, and the worklog had endorsed that justification as *"independently verified, CONFIRMED"*
- [[tasks/annotate-the-old-form-completion-flags-in-the-vault-log]] — task `0211` (Backlog board, unranked) → the flags `0173` **could not reach** because they were already emitted. Established two things beyond its own scope: **`log.md` is append-only with no exceptions** (owner-ruled, settling a collision flagged twice and unresolved twice), and **"describe, don't quote"** as the standing form for vault corrections
...
- [[decisions/adr-030-stop-hook-enforces-turn-completion-contract]] — the second hook, decided 2026-07-19 (not yet built) and not carried by a numbered row on this board
...
- [[tasks/the-2026-08-15-done-in-fact-wiki-closes]] — ⚠️ *Added 2026-08-22:* task `0238`, the vault re-sync filed against this board's archival, closed 2026-08-15 as `DONE-IN-FACT`
