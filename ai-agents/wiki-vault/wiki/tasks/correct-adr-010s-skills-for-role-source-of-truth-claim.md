...
- [[tasks/decide-whether-process-review-is-always-the-coder-or-the-architect-gains-the-skill]] — task `0200`, whose **primary evidence this task is**: its worker reported the hook denial verbatim, disclosed that it applied the method by hand and **never read the skill's procedure text**, and the re-run measured what that cost — **Steps 0/2/3/3.5 never ran and Step 4's status vocabulary was wrong**
- [[tasks/teach-record-decision-the-dated-correction-note-form]] — ⚠️ *Added 2026-08-22:* task `0198`, which encoded this task's snapshot caveat as a **two-command proof block**, because a working-tree diff cannot see an in-place edit made before the snapshot
- [[tasks/write-the-durable-citation-anchors-convention-page]] — ✅ *Added 2026-08-22:* task `0171`, **third link in the serialization recorded at line 38 above** (`0195` → `0196` → `0197` → `0171` → `0199`) — now **closed**. ⚠️ Its page does **not** supersede this task's *"permitted, not mandated"* residual on `:NNN` into a mutable file; the re-raise trigger recorded above is **not** fired by the close. ⛔ `0199` (the vault resync) remains the last link and is **still open**
