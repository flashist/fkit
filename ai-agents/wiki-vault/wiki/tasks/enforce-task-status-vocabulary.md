...
- [[tasks/add-an-exit-table-row-for-a-failed-build-verify-review-spawn-in-the-sprint-loop]] — task `0208`, whose three exit branches draw `🔲 Backlog`, `🔄 In progress` and `🚧 Blocked — <reason>` from this closed set — **nothing minted**. ⚠️ It also applies the vocabulary's own rule in the *opposite* direction: **a pause is not an exit**, so a paused task keeps `🔄 In progress` and no terminal status is written
- [[tasks/decide-what-the-sprint-driver-does-when-a-spawned-worker-dies]] — task `0167`, which chose both values from this set by reading it rather than inventing one — *a status is only true if it is current*
- [[tasks/gloss-the-moved-to-sprint-n-row]] — task `0268` (2026-08-13): the `➡️ Moved to [Sprint N]` row gains a gloss so **`N` reads as the sprint identity (`4`, or `4c`), not a number** — harmless while the parser matched only digits, **wrong the moment the letter suffix landed**
