...
- **`0151`** — `CLAUDE.md:43` names the wrong file for `skills_for_role()`, spotted while reading the skill tree. Unrelated to the banner. **`CLAUDE.md` is injected into every fkit session**, so every role in every session is currently told the wrong file — though it is a **one-hop misdirection, not a wrong edit** (the launcher documents the move right above the `source` line). Pointered into the fact-inventory investigation `0142` as a plausible live specimen.
  ...
  >
  > ✅ **Dated correction 2026-08-01 — `0160` is no longer open; it closed and it ruled.** *(agent-closed — not owner-verified.)* The sentence above is left byte-identical. **The ruling that applies to `CLAUDE.md:43`:** `path:NNN` stays correct for files under `claude/` and for code and tests — *edits arrive as a reviewed diff to the thing you cited* — so citing `CLAUDE.md:43` was not itself the defect. The rule that would have prevented the misdirection is the rider: **never cite a line number naked** — pair it with a quoted fragment or the heading it sits under, which makes a drifted pointer **self-correcting**. See [[tasks/decide-the-durable-citation-form-for-mutable-coordinates]]. ⚠️ **`0152` is still open** (`ai-agents/tasks/backlog/`) — the bullet above it is unchanged and remains current.
