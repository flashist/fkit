...
- [[systems/testing-and-verification]] — why a green suite proves nothing here: **no test reads a `SKILL.md` body**
  > ⚠️ **Dated correction 2026-08-29 (lint) — the line above is left byte-identical and is no longer true as an unqualified claim.** [[tasks/build-wiki-flag-convention-test]] (`0154`, 2026-08-28) landed `test/wiki-flag-convention.test.js`, ⭐ **the first test in the suite that reads a `SKILL.md` body at all.** ⛔ **What it does NOT change is this page's point:** the test reads **three named files only** — `fkit-wiki-ingest`, `fkit-wiki-sync`, `fkit-wiki-lint` — with the list **hard-coded on purpose and no `SKILL.md` walk** (`0154` ND-1). `claude/skills/fkit-sprint-ship-loop/SKILL.md`, this task's own file, is **still read by no test**, so *"a green suite proves nothing here"* stands exactly as written. Read the claim as **"no test reads THIS file's body"**.
