...
> ✅ **LINT 2026-08-29 — the `unverified` marker has since been REMOVED, and the removal was this task's own carve-out to hand over.** [[tasks/build-the-pretooluse-task-carry-check-hook-and-its-tests]] (`0204`, 2026-08-26) deleted it at **five sites** in the same change that landed the hook — `2 insertions / 22 deletions`, the two insertions being re-punctuation. ⛔ **One site was protected and edited surgically**: the paragraph headed *"The honest bound on 'true by construction'"* is the ADR-031 honesty clause for this whole construction, and only the now-false clause was cut — *"This construction narrows the hazard; it does not remove it"* is byte-untouched. ⚠️ **The paragraph above is left byte-identical as the record of what shipped here.**
