...
- This is the **doc-refresh half** of the ADR-028 fallout; the `PROJECT.md` half was split into task 83 per ADR-028 §*"Required follow-ups — none of them authorized by this ADR"*, item 1: *"(Owner's or producer's call — the brief is the product document, not the architect's.)"*
  > ⚠️ **Dated correction 2026-09-16 (the `b4a1a52`→`c59f4d7` sync). The bullet above previously cited `ADR-028:154`; that coordinate no longer lands.** ⛔ **Re-resolved on disk this sync: line 154 now carries a *"do not re-raise"* bullet, and the clause cited here sits under the §"Required follow-ups" heading.** ⭐ **Repaired to heading-plus-quoted-fragment**, per [[tasks/write-the-durable-citation-anchors-convention-page]] — ⭐ *"the durable anchors are the quoted text, not the numbers."* **Found by `0393`'s `E6` routing of the vault's `ADR-NNN:LINE` sites to `fkit-wiki`** ([[tasks/the-two-citation-sweeps-architecture-md-and-the-repo-wide-adr-line-class]]); ⛔ **this was one of only two of the vault's 28 sites triaged `drifted, live` — the other 25 are `mentioned` and were left byte-identical.**
