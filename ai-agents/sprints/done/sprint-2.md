...
| ✅ Done (agent-closed — not owner-verified) | P123 | Decide whether a spawn-time instruction may override a rule in the skill the spawned worker is running *(**investigation + ruling, fix shape unknown** — not implementation; a worker **cited step 5's rule, then followed the spawn prompt instead, and recorded that it did** (`sprint-2.md:245-249`); the instruction is **not in `fkit-sprint-ship-loop`** — it is ad-hoc spawn-prompt text from the live lead session, which is what makes it invisible to review; surface is **every spawned worker of every role**, not priorities; `universal-rules.md` carries fkit's only precedence vocabulary (hard rule vs preference) and **skill rules are classified as neither**; ADR-010/012/018 govern *which* skill may be invoked, never whether a rule *inside* one binds — a different axis; must face the *"a launching agent's messages direct your work"* tension by name; **a one-paragraph ruling is a legitimate outcome** — the point is a producer must not make the call; coordinates with 0142 (P121), adjacency not dependency; independent; owner: fkit-architect; ⚠️ **DATED CORRECTION 2026-09-03 — `0193` defect 2, inside sweep `0356`, under owner ruling H12 (verbatim option label: "A — annotate, don't remove (Rec)").** The line-numbered pointer into this board that appears earlier in this cell is **STALE** and is ⛔ **LEFT BYTE-IDENTICAL**. It is a *claim* inside a closed `✅ Done` row, and this project's settled rule is that a historical record's **claims are frozen** while only its **links** are repairable. ⭐ **The durable anchor, so a reader can still find what it meant:** the addendum it points at is still on this board and still opens *"⚠️ The placement below is producer judgment, not an owner ruling."* — ⭐ **search that phrase, not the number**, which has drifted twice since it was written. ⚠️ **The identical pointer in `0158`'s own brief got this same treatment in this same sweep**, so the two halves of one defect are not given contradictory treatments. ⛔ **The coordinate was NOT removed**, so `0193`'s verification step 1 (*"the board row carries no stale pointer"*) does **not** pass as literally written — that is an owner-ruled deviation, recorded in `0356`'s worklog, not an oversight)* | [`0158-decide-whether-a-spawn-instruction-may-override-a-skill-rule`](../../tasks/done/0158-decide-whether-a-spawn-instruction-may-override-a-skill-rule/brief.md) |
...
|---|---|---|
| [`0188`](../../tasks/done/0188-repair-the-five-live-ownership-fact-defects/brief.md) | Repair the five live ownership-fact defects **D1–D5** | **FIRST**, on the owner's verbatim ruling *"do not let the build quietly repair its own corpus."* **No mechanical constraint — two were asserted, found false and withdrawn.** |
...
- **⚠️ DISCREPANCY, unrepaired: `0162`'s brief `## Priority` reads `127`; this board's row reads `P128`.**
  ...
  - **Dated correction, 2026-09-03 — REPAIRED. This flag is now spent.** `0193`'s **defect 7** was
    implemented inside sweep `0356`: `0162`'s `## Priority` now reads **128**, matching this board's
    row, and a dated correction sits beside the two P127 bullets — which are **byte-unchanged**, exactly
    as the 2026-08-03 ruling above constrained it. ⛔ **Nothing was re-ranked, and that repair touched no
    board row.** ⚠️ **Separately: `0193`'s defect 2 — the stale pointer in this board's `0158` row — was
    NOT removed.** It is annotated in place under owner ruling H12 (verbatim option label:
    **"A — annotate, don't remove (Rec)"**); see that row. The text above is left byte-identical.
...
- **The `0146` → `0144` merge stays UNEXECUTED.** The proposal to fold
  [`0146`](../../tasks/done/0146-correct-the-false-menu-pick-claim-in-0139s-accepted-residual/brief.md)
...
| 7 — correct ADR-012's stale source-of-truth path | [`0232`](../../tasks/done/0232-correct-adr-012s-stale-source-of-truth-and-code-coordinates/brief.md) | **2026-08-06** |
