...
### ⭐ Addendum — the FIFTEENTH row, 2026-09-05: `0379`, appended at `P15`

**Owner instruction, given live in a `fkit lead` session on 2026-09-05. It was NOT a selection from an
option list, so the verbatim text is the owner's own sentence, not an option label:**
...
rather than a producer session. ⭐ **The intake questions stay** — *"after answering all the
questions"* keeps them; the complaint is the destination.

- ⭐ **THE CURRENT ROUTING IS DELIBERATE AND THE ROW SAYS SO.** Measured 2026-09-05:
  `claude/fkit-claude.sh`'s branch commented *"Fresh project: skip the menu, go straight to the
  producer's cold start"* runs `.fkit/interview` when executable, pins `role="producer"`, and
  `exec`s `claude --agent fkit-producer` with **one of two** seed strings (one for when
...
- ⛔⛔ **THE DESIGN FORK IS LEFT OPEN ON PURPOSE — the brief frames it, the plan gate decides it.**
  **A** lead-first with a spawned producer *(ADR-021 turns an interactive interview into
  return→relay→re-spawn rounds — the real technical risk)*; **B** producer-first then hand off to a
  lead session *(cheapest, keeps the live owner channel the initiation was written for, but the FIRST
  session is still the producer)*; **C** something else. ⚠️ **The owner's *"after answering all the
...
- ⛔ **APPENDED at `P15`, not inserted at merit position, and NOTHING was re-ranked.**
  ...
  belongs directly below `0359`**, because it is a user-facing behaviour change the owner asked for
  today and it should land **before** the release `0360` cuts, not nominally after it.
- ⛔ **Rank is not run order here either.** `0379` sits below `0360` (`P12`) in rank but **must run
  before it**, the same disagreement `0361`/`0360` already carries. The binding order lives in
  `Depends on` / `Blocks`.
- ⚠️ **`0360` (`P12`) now waits on `0379` too**, because its constraint *"Do not archive while any
  Sprint 7 row is open"* reaches every row on this board. ⛔ **`0360`'s `Depends on` field is
