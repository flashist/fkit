...
description: >-
  ...
  config changes take effect — never proactively or on a timer.
---
...
## Step 1 — Confirm this is really your own session, and capture your own true id
...
## Step 2 — Background the restart script and return immediately

Via `sys_os_shell`, run (as ONE command so `disown` applies to the job it just started):

```
nohup fkit restart-team "<your-session_id-from-step-1>" < /dev/null > /dev/null 2>&1 & disown; echo "restart launched"
```
...
## Step 3 — Tell the human what's happening, in the SAME reply, before ending your turn
...
recreated fresh under the new session; this chat's history and the old teammates' sessions are not
deleted, just no longer connected to anything." This must go out in the SAME turn as Step 2 — once
the process is killed, you cannot send a later message from it.
