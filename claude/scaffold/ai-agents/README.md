...
| Folder | Purpose |
|---|---|
| `sprints/` | Sprint plans (`sprint-N.md`) + the unranked `backlog.md` board for unsprinted work. A plan carries its own status in a **line-3 banner** — `🔲 Backlog` · `🔄 In progress` · `✅ Done` · `⛔ Cancelled` — and *"current sprint"* means **every** plan reading `🔄 In progress`. `Done` plans move to `sprints/done/` and `Cancelled` ones to `sprints/cancelled/`, **only** via the `/fkit-sprint-done` / `/fkit-sprint-cancelled` skills (never by hand). A plan with no line-3 banner resolves to `unresolved` and is never reported as active — add a banner to your open plans after upgrading. See [`sprint-status-vocabulary.md`](knowledge-base/conventions/sprint-status-vocabulary.md). |
