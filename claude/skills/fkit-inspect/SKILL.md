...
- Read `ai-agents/knowledge-base/PROJECT.md` if it exists, and skim any existing architecture notes,
  so you *augment* what is already known instead of re-deriving it. (For any wiki context, follow the
  read-only query procedure in `.claude/skills/fkit-query/SKILL.md` — reads are direct; only wiki
...
For a large codebase, read broadly yourself. You may delegate per-subsystem evidence-gathering to
**read-only research subagents** (e.g. the Explore agent type), one per subsystem, each gathering
evidence only and **never** asking the owner anything. This is an optional accelerant, not a
required step. Either way, keep every owner question in this main session and synthesize all
findings yourself.
