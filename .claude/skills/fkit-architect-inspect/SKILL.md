...
description: "Deeply and thoroughly research the current project architecture and write the findings to ai-agents/knowledge-base/architecture.md, asking the user as many clarifying questions as needed (never hesitate to ask). Use for an architecture deep-dive, an onboarding investigation, or documenting how the system fits together. Makes no commits."
user-invocable: true
---
...
**self-contained**: it researches the codebase and writes the document — nothing else. (Synthesizing
the result into the wiki, if you want that, is a separate step you run afterwards.)

## Ask early, ask often — never guess

**Ask the user as many questions as you need. DO NOT HESITATE TO ASK — there is no limit and no
penalty for asking.** Any time you are unsure about intent, scope, terminology, ownership, history, or
*why* something is the way it is, **stop and ask the user instead of assuming**. Batch related
questions so they can be answered efficiently, but keep going — read, ask, confirm, repeat — until you
genuinely understand the system. An unverified guess is a defect. When in doubt, ask.
...
- Read `ai-agents/knowledge-base/PROJECT.md` if it exists, and skim any existing architecture notes,
  so you *augment* what is already known instead of re-deriving it.
- If `ai-agents/knowledge-base/architecture.md` already exists, ask whether to **replace** or
  **extend** it before writing.
- Ask the user what to focus on and how deep to go: the whole system, a specific subsystem, or a
...
For a large codebase you may **fan out read-only research sub-agents** (via the Task/Agent tool), one per
subsystem, each gathering evidence only — they must **not** ask the user anything. Keep every user
question in this main session and synthesize the sub-agents' findings yourself.

## Step 2 — Interview the user to fill every gap

For everything the code cannot tell you — intent, history, constraints, trade-offs, the *why* behind
decisions, future direction, known pain points, and ownership — **ask the user**. Loop between reading
...
Ground every claim in either a code reference or an explicit user answer, and mark anything still
...
questions still outstanding. fkit **makes no commits** — everything is working-tree only.
