...
> Alex invokes it**, so it stays under their control. Invoking it is the signal that the task has been
...
- **Stop with a clear message if:**
  ...
  - it is in `ai-agents/tasks/done/` (it's already completed — this skill cancels *unfinished* work;
    **confirm with Alex** before cancelling something already marked done, rather than proceeding).
...
- The **cancellation reason** — the second parameter (the text after the path). **If it is empty,
  ask Alex for the reason before proceeding** — every cancelled entry in this project carries a
...
staging the move is enough; commits happen only when Alex explicitly asks.
...
- **List each dependent in the report** as "now affected — may need re-scoping or its own cancel,"
  so Alex can decide. Do not silently rewrite dependents; cancellation ripple is a judgment call.
...
- **No reference found** in any sprint doc: complete the move, then **report that no sprint status row
  was found** so Alex knows nothing else changed.
...
## Rules
- **Do not commit** anything (the project rule: commit only when Alex explicitly asks).
