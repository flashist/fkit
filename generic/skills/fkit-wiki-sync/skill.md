...
Sync the {{project_slug}} wiki at `{{wiki_path}}/` by detecting what changed in non-wiki `ai-agents/` sources since the last ingest and ingesting only the delta.
...
If the argument is `force`, skip to Step 2 and list all files under `ai-agents/` except `{{wiki_path}}/`.

Otherwise, check `{{wiki_path}}/.wiki-watermark` for a commit SHA written by the previous successful sync:
...
Run one of:
```
git log <sha>..HEAD --diff-filter=AMR --name-only --format="" -- ai-agents/ ':!{{wiki_path}}/'   # watermark mode
git log --since="<date>" --diff-filter=AMR --name-only --format="" -- ai-agents/ ':!{{wiki_path}}/'  # date-override mode
```

Or, for force / first-run: list all files under `ai-agents/` except `{{wiki_path}}/`.
...
Skip:
- `{{wiki_path}}/**` — wiki output, not an ingest source
...
Read `{{wiki_path}}/schema.md` for page templates and conventions.
Read `{{wiki_path}}/index.md` to know what pages already exist.
...
d. Update the one-line entry in `{{wiki_path}}/index.md` (add if new, update description if changed).
...
Run `git rev-parse HEAD` and write the resulting SHA to `{{wiki_path}}/.wiki-watermark` (overwrite, single line). This is the precise resume point for the next sync.

Append a single entry to `{{wiki_path}}/log.md`:
